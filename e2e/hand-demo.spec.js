...
// ─────────────────────────────────────────────────────────────────────────────
// Hand Demo module — live-browser layer (MODULE-TEST-CHECKLIST). Demos are shaped
...
// Stump Demo. NON-DESTRUCTIVE. Requires TEST_ESTIMATE_URL + a Hand Demo module on
...
test.describe('Hand Demo', () => {
  test.skip(!ESTIMATE, 'Set TEST_ESTIMATE_URL to a test estimate to enable Hand Demo checks.')

  test('module editor opens with demo sections', async ({ page }, testInfo) => {
    const errors = collectErrors(page)
    const ok = await openModule(page, 'Hand Demo')
    await testInfo.attach('hand-demo.png', { body: await page.screenshot({ fullPage: true }), contentType: 'image/png' })
    expect(ok, 'Could not open the Hand Demo module editor — check the add/edit flow.').toBeTruthy()
    ...
    expect(await anySection.count(), 'No Hand Demo sections rendered').toBeGreaterThan(0)
    expect(errors, `Console/HTTP errors:\n${errors.join('\n')}`).toEqual([])
  })

  test('any Type dropdown present is populated', async ({ page }) => {
    const ok = await openModule(page, 'Hand Demo')
    test.skip(!ok, 'Hand Demo editor not reachable on this estimate.')
  ...
  test('exhaustive: every dropdown option computes without a NaN/console error', async ({ page }, testInfo) => {
    ...
    await testInfo.attach('hand-demo-exhaustive.png', { body: await page.screenshot({ fullPage: true }), contentType: 'image/png' })
