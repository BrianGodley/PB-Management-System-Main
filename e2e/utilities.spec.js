...
// ─────────────────────────────────────────────────────────────────────────────
// Utilities module — live-browser layer of the MODULE-TEST-CHECKLIST. Paver areas with
...
//
// NON-DESTRUCTIVE: opens a test estimate + the Utilities editor, reads/enters values,
// NEVER saves. Requires TEST_ESTIMATE_URL + a Utilities module on the estimate.
// ─────────────────────────────────────────────────────────────────────────────
...
// like a pass. When the Utilities editor will not open, name the module rows that ARE on
// the estimate so the next run tells "Utilities is not on this estimate" (fix the estimate
// / TEST_ESTIMATE_URL) apart from "the row is named or marked up differently" (fix the
// selector), instead of skipping anonymously.
async function openUtilities(page) {
  const ok = await openModule(page, 'Utilities')
  if (ok) return { ok, why: '' }
  const titles = await moduleRowTitles(page)
  return {
    ok,
    why: `Utilities editor not reachable — module rows on this estimate: ${
...
test.describe('Utilities', () => {
  test.skip(!ESTIMATE, 'Set TEST_ESTIMATE_URL to a test estimate to enable Utilities checks.')

  test('module editor opens with utilities sections', async ({ page }, testInfo) => {
    const errors = collectErrors(page)
    const { ok, why } = await openUtilities(page)
    await testInfo.attach('utilities.png', { body: await page.screenshot({ fullPage: true }), contentType: 'image/png' })
    test.skip(!ok, why)
    const anySection = page.getByText(/Trench|Line|Gas|Electrical|Wire|Sewer|Fixture/i)
    expect(await anySection.count(), 'No Utilities sections rendered').toBeGreaterThan(0)
  ...
  test('exhaustive: every vendor × item option computes without NaN/console error', async ({ page }, testInfo) => {
    ...
    await testInfo.attach('utilities-exhaustive.png', { body: await page.screenshot({ fullPage: true }), contentType: 'image/png' })
  ...
  test('numeric fields accept input and the module computes a total', async ({ page }) => {
    ...
    // unpriced behavior itself is proven deterministically in utilitiesCalc.test.mjs.
  })
    ...
    for (const mode of [/^in.?house$/i, /^subcontractor$/i]) {
      ...
      await expect
        .poll(() => page.evaluate(() => /\bNaN\b|Infinity/.test(document.body.innerText)), {
          timeout: 8000,
          message: `Utilities mode ${mode} produced NaN/Infinity`,
        })
        .toBe(false)
    }
  ...
  test('live edit reflects: changing a field moves the total (Goal 4 in-browser)', async ({ page }) => {
    ...
    await expect(
      target,
      'No numeric input in the open Utilities editor — the module rendered no drivable field.'
    ).toHaveCount(1)
    ...
    await expect(target, 'Utilities field did not accept 100').toHaveValue('100')
    await page.waitForTimeout(600)
    const before = await dollars()
    await fillField(target, '900')
    await target.blur().catch(() => {})
    await expect(target, 'Utilities field did not accept 900').toHaveValue('900')
    const where = await target.evaluate(el => el.outerHTML.slice(0, 200))
    await expect
      .poll(dollars, {
        timeout: 10000,
        message: `Total did not change after editing a Utilities field 100 -> 900 (the input accepted the value, so this is a recompute/pricing issue, not a selector one). Input: ${where}`,
      })
      .not.toBe(before)
  })
})
