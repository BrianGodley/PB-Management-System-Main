#!/usr/bin/env node
/**
 * Hand Demo — consumed-rate manifest (checklist Goals 2 & 3 input). Static, no DB.
 *
 * Demo View Rates is DATA-DRIVEN: buildViewRates('Hand Demo') surfaces every
 ...
 *
 * Run:  node scripts/hand-demo-rate-coverage.mjs
 */
import { readFileSync } from 'node:fs'

const calc = readFileSync('src/components/modules/handDemoCalc.js', 'utf8')
...
console.log(`Hand Demo consumes ${lr.length + sr.length + mp.length} distinct rate keys.`)
show('labor_rates / misc_rates  (category Demo)', lr)
show('subcontractor_rates  (category Demo)', sr)
show('material (+ material_price)', mp)
console.log('\nVerify each exists + is priced with scripts/… DB-health SQL (hand-demo-db-health.sql).')
