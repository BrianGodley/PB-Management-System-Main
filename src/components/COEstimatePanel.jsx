...
export default function COEstimatePanel({
  ...
  async function updateProject() {
    ...
    if (updated) {
      setProjects(p =>
        p.map(pr => (pr.id === updated.id ? { ...pr, project_name: updated.project_name } : pr))
      )
      if (selectedProject?.id === updated.id)
        setSelectedProject(p => ({ ...p, project_name: updated.project_name }))
    }
    ...
    const updatedMods = await Promise.all(
      mods.map(async mod => {
        ...
        const updatedData = {
          ...(mod.data || {}),
          gpmd: newVal,
          calc: { ...(mod.data?.calc || {}), gp: newGP, commission: newComm, price: newPrice },
        }
        await supabase
  ...
  async function handleSaveCO() {
    ...
    if (!projects.length) {
      alert('Add at least one project before saving.')
      return
    }
    setSavingCO(true)
    try {
      ...
      if (bidId) {
        ...
        if (estimate?.client_id) {
          ...
          if (cd) {
            const parts = [
              cd.street,
              [cd.city, cd.state, cd.zip].filter(Boolean).join(', '),
            ].filter(Boolean)
            clientAddress = parts.join(', ')
          }
        }

        const { data: created, error: bidErr } = await supabase
          .from('bids')
          .insert({
  ...
  const estimateTotalSubGp = projects.reduce((sum, proj) => {
    ...
    return sum + projSubCost * (proj.sub_gp_markup_rate ?? 0.2)
  }, 0)
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <p
                        className={`text-sm font-semibold ${isSel ? 'text-blue-800' : 'text-gray-800'}`}
                      >
                        {proj.project_name}
                      </p>
