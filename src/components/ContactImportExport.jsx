...
const CONTACT_FIELDS = [
  ...
  { value: 'how_did_you_hear', label: 'Source Origin' },
  { value: 'date_of_birth', label: 'Date of Birth' },
  { value: 'interest_1', label: 'Interested In #1' },
  { value: 'interest_2', label: 'Interested In #2' },
  { value: 'interest_3', label: 'Interested In #3' },
...
const EXPORT_FIELDS = [
  ...
  { value: 'source', label: 'Source Type', on: false },
  { value: 'campaign', label: 'Campaign', on: false },
  { value: 'how_did_you_hear', label: 'Source Origin', on: false },
  { value: 'date_of_birth', label: 'Date of Birth', on: false },
  { value: 'interest_1', label: 'Interested In #1', on: false },
  { value: 'interest_2', label: 'Interested In #2', on: false },
  { value: 'interest_3', label: 'Interested In #3', on: false },
...
function autoDetect(header) {
  ...
  if (['name', 'fullname', 'contactname', 'clientname', 'customername'].includes(h))
    return 'full_name'
  ...
  if (['spousefirst', 'partnerfirst', 'secondaryfirst', 'spousepartnerfirst'].includes(h))
    return 'secondary_first_name'
  if (['spouselast', 'partnerlast', 'secondarylast', 'spousepartnerlast'].includes(h))
    return 'secondary_last_name'
  if (['company', 'companyname', 'organization', 'org', 'business', 'businessname'].includes(h))
    return 'company_name'
  if (['phone', 'phonenumber', 'homephone', 'telephone', 'tel', 'ph', 'workphone'].includes(h))
    return 'phone'
  if (['cell', 'cellphone', 'mobile', 'mobilephone', 'cellular', 'cellnumber'].includes(h))
    return 'cell'
  if (['email', 'emailaddress', 'mail', 'emailaddr'].includes(h)) return 'email'
  if (['address', 'streetaddress', 'street', 'addr', 'streetaddr'].includes(h))
  ...
  if (['companystreet', 'companyaddress', 'bizstreet', 'bizaddress'].includes(h))
  ...
  if (['source', 'leadsource', 'contactsource', 'referral', 'origin', 'howfound'].includes(h))
    return 'source'
  ...
  if (['projectdescription', 'projectdesc', 'project', 'projectdetails'].includes(h))
...
// ── Shared styles ─────────────────────────────────────────────────────────────
const btnPrimary =
  'px-4 py-2 bg-green-700 text-white rounded-lg text-sm font-semibold hover:bg-green-800 disabled:opacity-50 transition-colors'
const btnSecondary =
  'px-4 py-2 border border-gray-200 text-sm font-medium text-gray-600 rounded-lg hover:bg-gray-50 transition-colors'
...
export function ExportModal({ contacts, onClose, inline = false }) {
  ...
  const [selected, setSelected] = useState(
    new Set(EXPORT_FIELDS.filter(f => f.on).map(f => f.value))
  )
    ...
    try {
      ...
      const rows = contacts.map(c =>
        cols.map(col => {
          ...
          if (col.value === 'tags') return (c.tags || []).join(', ')
  ...
  const inner = (
    <div
      className={
        inline ? 'w-full max-w-lg' : 'bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6'
      }
    >
      <div className={`flex items-center justify-between mb-5 ${inline ? 'hidden' : ''}`}>
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            {contacts.length.toLocaleString()} contact{contacts.length !== 1 ? 's' : ''} selected
          </p>
        </div>
      ...
      {inline && (
        <p className="text-xs text-gray-400 mb-5">
          {contacts.length.toLocaleString()} contact{contacts.length !== 1 ? 's' : ''} will be
          exported
        </p>
      )}
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2">
          {[
            { v: 'csv', label: 'CSV', sub: 'Excel, Google Sheets' },
            { v: 'xlsx', label: 'Excel (.xlsx)', sub: 'Microsoft Excel' },
          ].map(f => (
            <button
              key={f.v}
      ...
      <div className="mb-5">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => setSelected(new Set(EXPORT_FIELDS.map(f => f.value)))}
      ...
      <div className="flex gap-3">
        {!inline && (
          <button onClick={onClose} className={btnSecondary + ' flex-1'}>
            Cancel
          </button>
        )}
        <button
  ...
  function handleFile(file) {
    ...
    reader.onload = e => {
      try {
        ...
        if (rows.length < 2) {
          setFileError('File appears to be empty or has only a header row.')
          return
        }
        ...
        if (hdrs.length === 0) {
          setFileError('Could not detect column headers in the first row.')
          return
        }
    ...
    <div
      className={
        inline
          ...
          : 'bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[92vh] flex flex-col'
      }
    >
      {/* Header */}
      <div
        className={`flex items-center justify-between ${inline ? 'mb-4' : 'px-6 pt-5 pb-4 border-b border-gray-100'} flex-shrink-0`}
      >
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            {[1, 2, 3, 4].map(n => (
              <div key={n} className="flex items-center gap-2">
                ...
                <span
                  className={`text-xs font-medium hidden sm:inline ${step === n ? 'text-gray-700' : 'text-gray-400'}`}
                >
            ...
            <div className="mt-5 p-4 bg-gray-50 rounded-xl border border-gray-200">
              ...
              <div className="flex flex-wrap gap-1.5">
                {[
                  ...
                  <span
                    key={f}
                    className="px-2 py-0.5 bg-white border border-gray-200 rounded text-xs text-gray-500"
                  >
                    {f}
                  </span>
                ))}
        ...
        {step === 2 && (
          <div>
            <p className="text-sm text-gray-500 mb-1">
              Match each column from your file to a contact field. Columns set to{' '}
              <strong>Skip</strong> will be ignored.
            </p>
            <p className="text-xs text-gray-400 mb-4">
              {fileRows.length} data row{fileRows.length !== 1 ? 's' : ''} detected · {mappedCount}{' '}
              column{mappedCount !== 1 ? 's' : ''} mapped
            </p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500 uppercase w-1/3">
                      File Column
                    </th>
                    <th className="text-left px-4 py-2 text-xs font-semibold text-gray-500 uppercase w-1/4">
                      Sample Value
                    </th>
        ...
        {step === 3 && preview && (
          <div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-5">
              ...
              <div
                className={`${preview.skipped.length > 0 ? 'bg-yellow-50 border-yellow-200' : 'bg-gray-50 border-gray-200'} border rounded-xl p-3 text-center`}
              >
                <p
                  className={`text-2xl font-bold ${preview.skipped.length > 0 ? 'text-yellow-600' : 'text-gray-400'}`}
                >
                  {preview.skipped.length}
                </p>
                <p
                  className={`text-xs font-medium mt-0.5 ${preview.skipped.length > 0 ? 'text-yellow-600' : 'text-gray-400'}`}
                >
                  Skipped (duplicates)
                </p>
              </div>
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 text-center">
                <p className="text-2xl font-bold text-gray-600">
                  {preview.toImport.length + preview.skipped.length}
                </p>
            ...
            {preview.toImport.length > 0 && (
              <div className="mb-4">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
                  Preview — first {Math.min(SAMPLE, preview.toImport.length)} of{' '}
                    ...
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        {['Name', 'Company', 'Phone', 'Email', 'City', 'Stage'].map(h => (
                          <th
                            key={h}
                            className="text-left px-3 py-2 font-semibold text-gray-500 uppercase whitespace-nowrap"
                          >
                            {h}
                          </th>
                        ))}
            ...
            {preview.skipped.length > 0 && (
              <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-xl">
                <p className="text-xs font-semibold text-yellow-700 mb-1">
                  ⚠ {preview.skipped.length} row{preview.skipped.length !== 1 ? 's' : ''} will be
                  skipped — duplicate email or phone already in your contacts.
                </p>
                <div className="text-xs text-yellow-600 space-y-0.5 max-h-20 overflow-y-auto">
                  ...
                  {preview.skipped.length > 5 && (
                    <p className="text-yellow-500">…and {preview.skipped.length - 5} more</p>
                  )}
                </div>
              </div>
            )}
      ...
      {/* Footer actions */}
      <div
        className={`flex gap-3 ${inline ? 'pt-4' : 'px-6 py-4 border-t border-gray-100'} flex-shrink-0`}
      >
        {step === 1 && !inline && (
          <button onClick={onClose} className={btnSecondary + ' flex-1'}>
            Cancel
        ...
        {step === 3 && (
          <>
            <button onClick={() => setStep(2)} className={btnSecondary} disabled={loading}>
              ← Back
            </button>
            <button
              onClick={doImport}
            ...
            >
              {loading
                ? 'Importing…'
                : `Import ${preview?.toImport.length ?? 0} Contact${preview?.toImport.length !== 1 ? 's' : ''}`}
            </button>
          </>
        )}
