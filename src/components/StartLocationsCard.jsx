...
export default function StartLocationsCard({ currentUserIsAdmin = true }) {
  ...
  async function load() {
    ...
    const { data } = await supabase.from('company_settings')
      .select('start_locations, default_start_location_id').maybeSingle()
  ...
  async function persist(nextLocations, nextDefaultId = defaultId) {
    const updates = { start_locations: nextLocations, default_start_location_id: nextDefaultId || null }
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      ...
      <p className="text-sm text-gray-500 mb-4">
        Saved starting points for route optimization (e.g. Main Office, North Yard, a supervisor's home).
        The optimizer pre-selects the default but you can pick a different one each session.
      </p>
      ...
      {loading ? (
        ...
        <>
          <div className="space-y-1.5 mb-4">
            {locations.length === 0 && (
              <p className="text-xs text-gray-400 italic">No start locations yet — add one below.</p>
            )}
            {locations.map(loc => (
              <div key={loc.id} className="flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-200 bg-gray-50">
                <button
                ...
                {currentUserIsAdmin && (
                  <button onClick={() => removeLocation(loc.id)} className="text-red-300 hover:text-red-600 text-xs px-1.5">✕</button>
                )}
              </div>
            ))}
          </div>
