...
export default function SupervisorPositionsCard() {
  ...
  async function load() {
    ...
    const ids = Array.isArray(settingsRes.data?.supervisor_position_ids)
      ? settingsRes.data.supervisor_position_ids
      : []
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      ...
      <p className="text-sm text-gray-500 mb-4">
        Pick which HR positions count as supervisors for the Schedule Assistant optimizer. The
        optimizer will cluster open jobs across active employees holding any of these positions.
      </p>
      ...
      {loading ? (
        ...
        <>
            ...
            {positions.length === 0 ? (
              ...
              positions.map(p => {
                ...
                return (
                  <label
                    key={p.id}
                    className={`flex items-center gap-2 px-3 py-2 text-sm border-b border-gray-100 last:border-b-0 cursor-pointer hover:bg-gray-50 ${!checked ? 'opacity-60' : ''}`}
                  >
                    <input
                      type="checkbox"
                      checked={checked}
                    ...
                    <span className="text-[11px] text-gray-400">
                      {count > 0
                        ? `${count} active employee${count === 1 ? '' : 's'}`
                        : 'no employees'}
                    </span>
                  </label>
                )
          ...
          <div className="flex items-center gap-3">
            ...
            {savedMsg && (
              <span
                className={`text-xs font-semibold ${savedMsg.startsWith('Error') ? 'text-red-600' : 'text-green-700'}`}
              >
                {savedMsg}
              </span>
            )}
