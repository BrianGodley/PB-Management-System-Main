...
function makeListHelpers(setList) {
  return {
    add(nameField, value, keyPrefix = '') {
      ...
      setList(prev => [
        ...prev,
        { _key: `${keyPrefix}new-${Date.now()}`, [nameField]: trimmed, sort_order: prev.length },
      ])
    },
    remove(key) {
      ...
      {folders.length > 0 && (
        <div className="border border-gray-200 rounded-lg overflow-hidden mb-2">
          {folders.map((f, i) => (
            <div
              key={f._key}
              className="flex items-center gap-2 px-3 py-2 border-b border-gray-100 last:border-b-0 bg-white hover:bg-gray-50"
            >
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Templates define document folders, photo/video folders, and tasks applied when a new job
            is created.
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-8 text-center text-gray-400">
          ...
          <p className="text-xs mt-1">
            Create a template to define folder structures and tasks for new jobs
          </p>
        </div>
      ) : (
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {templates.map(t => {
                const docCount = (t.template_folders || []).filter(
                  f => f.folder_type !== 'photo_video'
                ).length
                const photoCount = (t.template_folders || []).filter(
                  f => f.folder_type === 'photo_video'
                ).length
                ...
                return (
                  <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {t.auto_trigger ? (
                        <span className="text-xs px-2 py-0.5 rounded-full bg-green-100 text-green-700 font-medium">
                          {AUTO_TRIGGER_LABELS[t.auto_trigger] || t.auto_trigger}
                        </span>
                      ) : (
                    ...
                    <td className="px-4 py-3 text-center text-xs text-gray-600">
                      {docCount || <span className="text-gray-300">—</span>}
                    </td>
                    <td className="px-4 py-3 text-center text-xs text-gray-600">
                      {photoCount || <span className="text-gray-300">—</span>}
                    </td>
                    <td className="px-4 py-3 text-center text-xs text-gray-600">
                      {taskCount || <span className="text-gray-300">—</span>}
                    </td>
                    <td className="px-4 py-3">
...
function TemplateModal({ template, userId, onSave, onClose }) {
  ...
  async function handleSave() {
    ...
    if (allFolders.length) {
      ...
      if (folderErr)
        console.warn('Folder insert error (run supabase-update-73.sql?):', folderErr.message)
    }
      ...
      >
        ...
        <div className="bg-green-700 px-6 py-4 flex-shrink-0">
          <h2 className="text-white font-bold text-lg">
            {isEdit ? 'Edit Template' : 'New Job Template'}
          </h2>
        </div>
        ...
        <div className="px-6 py-5 overflow-y-auto space-y-5">
          ...
          <div>
            ...
            {form.auto_trigger === 'sold_bid' && (
              <p className="text-xs text-green-700 mt-1">
                ✅ Folders and tasks will be created automatically when a bid is marked Sold.
              </p>
            )}
          </div>
          ...
          <div className="border-t border-gray-100 pt-1">
            <p className="text-[11px] text-gray-400 mb-4">
              Items below are created on every job that uses this template.
            </p>
            ...
            <div className="space-y-5">
              <FolderSection
                ...
                folders={docFolders}
                helpers={docH}
                newName={newDocFolder}
                setNewName={setNewDocFolder}
              />

              {/* Photo / Video Folders */}
              <FolderSection
                icon="📸"
                label="Photo / Video Folders"
                folders={photoFolders}
                helpers={photoH}
                newName={newPhotoFolder}
                setNewName={setNewPhotoFolder}
              />
