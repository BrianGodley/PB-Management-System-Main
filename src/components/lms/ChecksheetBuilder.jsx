...
const STEP_TYPES = [
  ...
  {
    value: 'learning_drill',
    label: 'Learning Drill',
    icon: '🔁',
...
function StepBadge({ type }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${t.color || 'bg-gray-100 text-gray-600'}`}
    >
      {t.icon} {t.label}
    </span>
  )
...
export default function ChecksheetBuilder({ course: initialCourse, draft, onClose, onSaved }) {
  ...
  const openEditStep = step => {
    setStepForm({
      ...
      read_item_id: step.read_item_id || '',
      learning_drill_id: step.learning_drill_id || '',
  ...
  const saveStep = async () => {
    ...
    if (stepForm.step_type === 'read' && uploadMode === 'upload' && uploadFile) {
      try {
        ...
        setStepError(
          err.message || 'Upload failed. Check that the lms-documents storage bucket exists.'
        )
        setSavingStep(false)
        return
      }
    ...
    if (editingStep?.id) {
      ...
      setSteps(prev => [
        ...prev,
        { ...payload, id: crypto.randomUUID(), _new: true, step_order: prev.length },
      ])
    }
    setSavingStep(false)
    setUploadFile(null)
  ...
  const saveChecksheet = async () => {
    ...
    if (steps.length > 0) {
      await supabase.from('lms_steps').insert(
        steps.map((s, i) => ({
          course_id: courseId,
          step_order: i,
          ...
          read_item_id: s.read_item_id || null,
          learning_drill_id: s.learning_drill_id || null,
              ...
              <div>
                ...
                {uploadFile && (
                  <p className="text-xs text-green-600 mt-1.5">
                    ✓ Document will be saved when you click "Add Step"
                  </p>
                )}
              </div>
            ) : (
      ...
      case 'learning_drill':
        return (
          <div>
            ...
            {drills.length === 0 ? (
              warn('No learning drills yet — create some in the Learning Drills tab first.')
            ) : (
              <select
                value={sf.learning_drill_id}
                onChange={e => setSF('learning_drill_id', e.target.value)}
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-green-500"
              >
      ...
      case 'quiz':
        return (
          <div>
            ...
            {quizzes.length === 0 ? (
              warn('No quizzes yet — create some in the Quizzes tab first.')
            ) : (
              <select
                value={sf.quiz_id}
                onChange={e => setSF('quiz_id', e.target.value)}
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-green-500"
              >
      ...
      case 'final_test':
        return (
          <div>
            ...
            {tests.length === 0 ? (
              warn('No final tests yet — create some in the Final Tests tab first.')
            ) : (
              <select
                value={sf.test_id}
                onChange={e => setSF('test_id', e.target.value)}
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-green-500"
              >
      ...
      case 'action':
        return (
          <div>
            ...
            {actions.length === 0 ? (
              warn('No actions yet — create some in the Actions tab first.')
            ) : (
              <select
                value={sf.action_id}
                onChange={e => setSF('action_id', e.target.value)}
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-green-500"
              >
        ...
        <div className="w-72 bg-white border-r border-gray-200 p-5 space-y-4 overflow-y-auto flex-shrink-0">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Safety, Installation…"
              className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-green-500"
            />
          </div>
          <div>
        ...
        <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {STEP_TYPES.map(t => (
                  <button
                    key={t.value}
                    onClick={() => setSF('step_type', t.value)}
            ...
            <div className="flex gap-3 pt-2">
              <button
                onClick={saveStep}
                disabled={!stepForm.title.trim() || savingStep}
