    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-500 text-sm">
          You've used all {maxAttempts} attempt{maxAttempts !== 1 ? 's' : ''}. Please speak with
          your trainer.
        </p>
      </div>
    )
  ...
  if (submitted) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="font-bold text-gray-900 text-xl mb-1">{passed ? 'Passed!' : 'Not quite'}</h3>
        <p className="text-gray-500 mb-2">
          Your score:{' '}
          <span className={`font-bold text-lg ${passed ? 'text-green-600' : 'text-red-500'}`}>
            {score}%
          </span>
        </p>
...
export default function CoursePlayer({ assignment, onClose }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const incomplete = (stepsData || []).findIndex(
        s => !(compData || []).some(c => c.step_id === s.id)
      )
  ...
  const completeStep = async (stepId, score = null) => {
    await supabase
      .from('lms_step_completions')
      .upsert(
        { assignment_id: assignment.id, step_id: stepId, score },
  ...
  const recordAttempt = async (stepId, score, passed) => {
    const { data } = await supabase
      .from('lms_quiz_attempts')
      .insert({ assignment_id: assignment.id, step_id: stepId, score, passed })
      .select()
    ...
    return (
      <div className="fixed inset-0 bg-gray-50 z-50 flex flex-col items-center justify-center p-8">
        ...
        <p className="text-gray-500 mb-8 text-center">
          You've completed all steps in this checksheet.
        </p>
        <button
          onClick={onClose}
          className="px-8 py-3 bg-green-700 text-white rounded-2xl font-bold text-lg hover:bg-green-800"
        >
          Done
        </button>
      </div>
  ...
  const renderStepContent = () => {
    switch (step.step_type) {
              ...
              >
                ...
                <div>
                  <p className="font-semibold text-blue-800">
                    {step.read_item?.title || 'Open Document'}
                  </p>
                  <p className="text-sm text-blue-600 mt-0.5">
                    Click to open and read — then mark as read
                  </p>
                </div>
              </button>
            ) : (
              <p className="text-sm text-gray-400 italic">
                No document attached to this read item.
              </p>
            )}
            {!stepDone && step.read_item?.doc_url && (
              <p className="text-xs text-gray-400 text-center">
                Open the document above, then click "Completed" when you're done reading.
              </p>
            )}
          </div>
        )
          ...
          <div className="space-y-4">
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-2xl p-5">
              <p className="text-gray-800 whitespace-pre-wrap">
                {step.learning_drill?.content || 'No content available.'}
              </p>
            </div>
            {!stepDone && (
              <button
          ...
          return (
            <div className="text-center py-10">
              ...
              <p className="text-gray-400 text-sm mt-1">
                Score: {stepAttempts.find(a => a.passed)?.score ?? '—'}%
              </p>
            </div>
          )
        return (
          ...
          <div className="space-y-4">
            <div className="bg-pink-50 border border-pink-200 rounded-2xl p-5">
              ...
              <p className="text-gray-700 whitespace-pre-wrap">
                {step.action?.instructions || 'No instructions provided.'}
              </p>
            </div>
            {!stepDone && (
              <button
      ...
      default:
        return <p className="text-gray-400">Unknown step type.</p>
    }
  }
    ...
    <div className="fixed inset-0 bg-gray-50 z-50 flex flex-col">
        ...
        <div className="flex items-center gap-4 mb-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500">
              {completions.size} of {steps.length} steps complete
            </p>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mb-6">
            <span
              className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-medium ${tm.color || 'bg-gray-100 text-gray-600'}`}
            >
              {tm.icon} {tm.label}
            </span>
