...
export default function CourseAssignmentsManager() {
  ...
  const assign = async () => {
    ...
    const { error } = await supabase
      .from('lms_assignments')
      .upsert(
        { employee_id: selEmployee, course_id: selCourse },
  ...
  const remove = async id => {
    if (!confirm("Remove this course assignment? The employee's progress will also be deleted."))
      return
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-xs text-gray-500 mt-0.5">
          Assign one or more courses to an employee. Assigned courses appear in their Training
          module.
        </p>
      </div>
              ...
              >
                ...
                <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 border-b border-gray-100">
                  ...
                  <div>
                    ...
                    {employee?.job_title && (
                      <p className="text-xs text-gray-400">{employee.job_title}</p>
                    )}
                  </div>
                  <span className="ml-auto text-xs text-gray-400">
                    {empCourses.length} course{empCourses.length !== 1 ? 's' : ''}
                  </span>
                </div>
                {/* Course rows */}
                {empCourses.map(a => (
