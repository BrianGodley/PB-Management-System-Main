  ...
  const save = async () => {
    ...
    if (file) {
      ...
      if (!upErr) {
        const {
          data: { publicUrl },
        } = supabase.storage.from('lms-documents').getPublicUrl(path)
        doc_url = publicUrl
        file_name = file.name
      }
      setUploading(false)
    ...
    if (editing) {
      await supabase
        .from('lms_read_items')
        .update({ ...form, doc_url, file_name })
        .eq('id', editing.id)
    } else {
      await supabase
        .from('lms_read_items')
        .insert({ ...form, doc_url, file_name, created_by_email: user?.email })
    }
    setSaving(false)
    setShowModal(false)
    load()
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Upload documents that employees will read as part of a checksheet step.
          </p>
        </div>
        <button
          onClick={openAdd}
          className="px-3 py-1.5 bg-green-700 text-white text-sm rounded-lg hover:bg-green-800"
        >
          + Add Read Item
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 space-y-4">
            <h3 className="font-bold text-gray-900 text-lg">
              {editing ? 'Edit Read Item' : 'New Read Item'}
            </h3>
            ...
            <div>
              ...
              {editing?.doc_url && !file && (
                <p className="text-xs text-gray-500 mb-2">
                  Current:{' '}
                  <a
                    href={editing.doc_url}
                    target="_blank"
                    rel="noreferrer"
                    className="text-blue-600 hover:underline"
                  >
                    {editing.file_name || 'View file'}
                  </a>
                </p>
              )}
              <div
