    ...
    <div className="space-y-1 text-sm">
        ...
        <>
          ...
          {baseRows.map((r, i) => {
            ...
            return (
              <LineRow
                key={i}
                label={`${r.label} — ${n(r.sf).toLocaleString()} SF @ ${r.depth}"`}
                value={fh(c.hrs) || '—'}
      ...
      {n(installSF) > 0 && (
        <>
          ...
          {installHrs > 0 && (
            <LineRow
              label={`Pour + Finish — ${n(installSF).toLocaleString()} SF @ ${depthIn}"`}
              value={fh(installHrs) || '—'}
          ...
          {rebarHrs > 0 && (
            <LineRow
              label={`Rebar 24" OC — ${n(rebarSF).toLocaleString()} SF`}
              value={fh(rebarHrs) || '—'}
          ...
          {formHrs > 0 && (
            <LineRow
              label={`Form Edging — ${n(formLF).toLocaleString()} LF`}
              value={fh(formHrs) || '—'}
          ...
          {sleeveHrs > 0 && (
            <LineRow
              label={`3" Sleeves — ${n(sleeveLF).toLocaleString()} LF`}
              value={fh(sleeveHrs) || '—'}
      ...
      {(colorYes || pumpYes || n(vaporBarrierSF) > 0 || n(sealerSF) > 0) && (
        <>
          ...
          {vaporHrs > 0 && (
            <LineRow
              label={`Vapor Barrier — ${n(vaporBarrierSF).toLocaleString()} SF`}
              value={fh(vaporHrs) || '—'}
          ...
          {sealerHrs > 0 && (
            <LineRow
              label={`Sealer (${sealerType}) — ${n(sealerSF).toLocaleString()} SF`}
              value={fh(sealerHrs) || '—'}
      ...
      {/* Travel / Backyard */}
      {(n(layoutHrs) > 0 ||
        n(savedCalc.travelHrs) > 0 ||
        n(savedCalc.backyardHrs) > 0 ||
        n(savedCalc.complexityHrs) > 0) && (
        <>
          <SectionLabel title="Adjustments" />
          {n(layoutHrs) > 0 && <LineRow label="Layout Time" value={fh(n(layoutHrs)) || '—'} />}
          {n(savedCalc.travelHrs) > 0 && (
            <LineRow
              label="Travel (truck distance)"
              value={fh(n(savedCalc.travelHrs)) || '—'}
              sub={`${distanceLF} LF from truck`}
            />
          )}
          {n(savedCalc.backyardHrs) > 0 && (
            <LineRow
              label="Backyard Congestion"
              value={fh(n(savedCalc.backyardHrs)) || '—'}
              sub={`${pctBackyard}% in backyard`}
            />
          )}
          {n(savedCalc.complexityHrs) > 0 && (
            <LineRow label="Forming Complexity" value={fh(n(savedCalc.complexityHrs)) || '—'} />
          )}
        </>
      )}
