...
function calcDemo(state, laborRatePerHour, materialPrices, laborRates, subMarkupRate = 0.35, subRates = {}, gpmd = 425, walkAccess = null) {
  ...
  return {
    ...
    gradeCut, gradeFill, jjTons, jjHrs,
    rebarHrs,
    shrubHrs, stumpFstHrs, stumpAddHrs,
    treeCalc,
    crewDemoHrs, gradingHrs, vegHrs, manualHrs,
    dumpMatCost, isSub, subHaulCost,
    shConc, shDirt, shGrass,
    // expose resolved rates
    rateConc, rateGrass, rateBase, rateBucket, rateJJ, rebarMinPerSF,
    shrubRate, stumpFstRate, stumpAddRate,
...
const DEFAULT_STATE = {
  ...
  // Hand Bucket Areas
  bucketRows: Array(4).fill(null).map(() => ({ label: '', sf: '', depth: 4 })),
  // Grading
        ...
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-gray-50">
            {state.bucketRows.map((r,i)=>{
              ...
              return (<tr key={i}>
                <td className={td}><Inp type="text" value={r.label} onChange={e=>setRow('bucketRows',i,'label',e.target.value)} placeholder={`Area ${i+1}`} /></td>
                <td className={td}><Inp value={r.sf}    onChange={e=>setRow('bucketRows',i,'sf',   e.target.value)} /></td>
                <td className={td}><Inp value={r.depth} onChange={e=>setRow('bucketRows',i,'depth',e.target.value)} placeholder="4" /></td>
                <td className={num}>{cr.tons>0?cr.tons.toFixed(1):'—'}</td>
