    ...
    <div className="space-y-4">
                ...
                <p className="text-[10px] uppercase tracking-wide font-bold text-gray-400 px-2 pt-1 pb-0.5">Switch to</p>
                  ...
                  <button
                    onClick={() => {
                      ...
                      // up to EstimateDetail so the target module loads
                      // with everything prefilled.
                      onSwitchType?.('Skid Steer Demo', { ...state, materialPrices, laborRates, laborRatePerHour, subMarkupRate, subRates })
                      setShowDemoSwitcher(false)
                    }}
                    className="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-green-50 hover:text-green-800 rounded transition-colors"
                  >
          ...
          {!isSelf && (
            <p className="text-xs text-amber-600 mt-0.5 inline-flex items-center flex-wrap gap-x-1 gap-y-0.5">
              ...
              <span className="inline-flex items-center gap-0.5">${calc.shConc}/1.5T conc
                <RateEditPopover table="subcontractor_rates" name="Sub Haul - Concrete" unitLabel="/1.5T"
                  currentValue={calc.shConc} onSaved={refreshAllRates} />
              </span>·
              <span className="inline-flex items-center gap-0.5">${calc.shDirt}/1.5T dirt
                <RateEditPopover table="subcontractor_rates" name="Sub Haul - Dirt" unitLabel="/1.5T"
                  currentValue={calc.shDirt} onSaved={refreshAllRates} />
              </span>·
              <span className="inline-flex items-center gap-0.5">${calc.shGrass}/1.5T grass
                <RateEditPopover table="subcontractor_rates" name="Sub Haul - Grass" unitLabel="/1.5T"
                  currentValue={calc.shGrass} onSaved={refreshAllRates} />
              </span>
            </p>
          )}
        </div>
      ...
      <div>
        ...
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-gray-50">
            {[
              {label:'Concrete',    sfK:'concSF',  dK:'concDepth',  dep:4, row:calc.conc,  fee:dumpConc,  rate: calc.rateConc,  rateName: 'Demo - Mini Skid Steer Concrete/Dirt', rateNote:`${calc.rateConc} t/hr`},
              {label:'Dirt/Rock',   sfK:'dirtSF',  dK:'dirtDepth',  dep:6, row:calc.dirt,  fee:dumpDirt,  rate: calc.rateConc,  rateName: 'Demo - Mini Skid Steer Concrete/Dirt', rateNote:`${calc.rateConc} t/hr`},
              {label:'Import Base', sfK:'baseSF',  dK:'baseDepth',  dep:4, row:calc.base,  fee:dumpBase,  rate: calc.rateBase,  rateName: 'Demo - Mini Skid Steer Import Base',   rateNote:`${calc.rateBase} t/hr · $${dumpBase}/ton base`, extraIcon: (
                <RateEditPopover table="material_rates" name="Dump Fee - Import Base" category="Demo"
                  unitLabel="ton" currentValue={dumpBase} onSaved={refreshAllRates} />
              )},
              {label:'Grass/Sod',   sfK:'grassSF', dK:'grassDepth', dep:2, row:calc.grass, fee:dumpGreen, rate: calc.rateGrass, rateName: 'Demo - Mini Skid Steer Grass',         rateNote:`${calc.rateGrass} t/hr · $${dumpGreen}/ton green waste`, extraIcon: (
        ...
        <div className="mt-3 flex items-center gap-3">
          <div className="flex-1 max-w-xs">
            <p className="text-xs text-gray-500 mb-0.5 inline-flex items-center gap-1">
              ...
              <RateEditPopover table="labor_rates" name="Demo - Rebar" category="Demo"
      ...
      <div>
        ...
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-gray-50">
            {[
              {label:'Grade Cut',                sfK:'gradeCutSF',  dK:'gradeCutDepth',  dep:3, tons:calc.gradeCut.tons,  hrs:calc.gradeCut.hours,  note:`${calc.rateConc} t/hr`,  rate: calc.rateConc,  rateName: 'Demo - Mini Skid Steer Concrete/Dirt'},
              {label:'Grade Fill',               sfK:'gradeFillSF', dK:'gradeFillDepth', dep:3, tons:calc.gradeFill.tons, hrs:calc.gradeFill.hours, note:`${calc.rateBase} t/hr`,  rate: calc.rateBase,  rateName: 'Demo - Mini Skid Steer Import Base'},
              ...
              {label:'Mini SS Compact (4" Max)', sfK:'ssCmpSF',     dK:'ssCmpDepth',     dep:3, tons:calc.ssCmpTons,      hrs:calc.ssCmpHrs,        note:`${calc.rateSSCmp} t/hr`, rate: calc.rateSSCmp, rateName: 'Demo - Mini SS Compaction'},
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {[
          {label:'Shrubs (qty)',       key:'shrubQty',      hrs:calc.shrubHrs,    sub:`${calc.accessBobcat}× × ${calc.shrubRate} hrs/ea`, rate: calc.shrubRate,    rateName: 'Demo - Shrub'},
          {label:'Stump Grind 1st',    key:'stumpFirstQty', hrs:calc.stumpFstHrs, sub:`${calc.accessBobcat}× × ${calc.stumpFstRate} hrs`, rate: calc.stumpFstRate, rateName: 'Demo - Stump 1st'},
          {label:"Stump Grind Add'l",  key:'stumpAddQty',   hrs:calc.stumpAddHrs, sub:`${calc.accessBobcat}× × ${calc.stumpAddRate} hrs`, rate: calc.stumpAddRate, rateName: 'Demo - Stump Additional'},
          ...
          {isSelf && (
            <span className="font-normal normal-case text-gray-400 inline-flex items-center gap-1">
              ...
              <RateEditPopover table="material_rates" name="Dump Fee - Tree/Stump" category="Demo"
                unitLabel="ton" currentValue={dumpTreeStump} onSaved={refreshAllRates} />
            </span>
          )}
        </div>
        <table className="w-full text-xs">
