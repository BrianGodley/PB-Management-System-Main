// ─────────────────────────────────────────────────────────────────────────────
// MiniSkidSteerDemoSummary — read-only detail view for a saved Mini Skid Steer Demo module
...
// Fallback defaults matching RATE_DEFAULTS in the module
const R = {
  concrete: 0.75,
  grass: 0.75,
  importBase: 5.0,
...
export default function MiniSkidSteerDemoSummary({ module }) {
  ...
  // ── Resolve rates from saved snapshot ────────────────────────────────────
  ...
  const miscVertCalc = miscVertRows.map(r =>
    vert(r.lf, r.heightIn || 0, r.widthIn || 8, rateConc, dumpConc)
  )
  ...
  const ssCmpTons = sfToTons(ssCmpSF, ssCmpDepth || 3)
  ...
  const treeCalc = treeRows.map(r => {
    ...
    const dumpFee = isSelf ? tons * dumpTreeStump : 0
  ...
  const hasLines =
    ...
    grass.tons ||
    n(rebarSF) ||
    miscFlatCalc.some(r => r.tons) ||
    miscVertCalc.some(r => r.tons) ||
    footingCalc.some(r => r.tons) ||
    gradeCut.tons ||
    gradeFill.tons ||
    jjTons ||
    ssCmpTons ||
    n(shrubQty) ||
    treeCalc.some(r => r.hrs) ||
    ...
    <div className="space-y-1 text-sm">
      ...
      <div className="flex gap-2 flex-wrap mb-1">
        <span className="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded">
          {access} access ({accessMult}×)
        </span>
        <span
          className={`text-xs px-2 py-0.5 rounded ${isSelf ? 'bg-gray-100 text-gray-700' : 'bg-amber-50 text-amber-700'}`}
        >
          {dumpType}
        </span>
        {n(difficulty) > 0 && (
      ...
      {!hasLines ? (
        ...
        <>
          ...
          {(conc.tons || dirt.tons || base.tons || grass.tons) > 0 && (
            <>
              ...
              {[
                ...
                { label: 'Import Base', r: base, fee: dumpBase },
              ...
              {miscFlatRows.map((r, i) => {
                ...
                return (
                  <LineRow
                    ...
                    value={fh(cr.hours) || '—'}
                    sub={`${n(r.sf)} SF × ${r.depth || 4}"  ·  ${cr.tons.toFixed(1)} tons${isSelf && cr.dumpFee > 0 ? `  ·  dump ${fmt2(cr.dumpFee)}` : ''}`}
                  />
                )
              })}
            </>
              ...
              {miscVertRows.map((r, i) => {
                ...
                return (
                  <LineRow
                    ...
                    label={r.label || `Item ${i + 1}`}
                    value={fh(cr.hours) || '—'}
                    sub={`${n(r.lf)} LF × ${r.heightIn || 0}" × ${r.widthIn || 8}"  ·  ${cr.tons.toFixed(2)} tons${isSelf && cr.dumpFee > 0 ? `  ·  dump ${fmt2(cr.dumpFee)}` : ''}`}
                  />
                )
              })}
            </>
            ...
            <>
              ...
              {footingRows.map((r, i) => {
                ...
                return (
                  <LineRow
                    key={i}
                    label={r.label || `Footing ${i + 1}`}
                    value={fh(cr.hours) || '—'}
                    sub={`${n(r.sf)} SF × ${r.depth || 12}"  ·  ${cr.tons.toFixed(2)} tons${isSelf && cr.dumpFee > 0 ? `  ·  dump ${fmt2(cr.dumpFee)}` : ''}`}
                  />
                )
              })}
            </>
          ...
          {/* Grading */}
          {(gradeCut.tons || gradeFill.tons || jjTons || ssCmpTons) > 0 && (
            <>
              <SectionLabel title="Grading" />
              {gradeCut.tons > 0 && (
                <LineRow
                  label="Grade Cut"
                  value={fh(gradeCut.hours) || '—'}
                  sub={`${gradeCut.tons.toFixed(1)} tons  ·  ${rateConc} t/hr`}
                />
              )}
              {gradeFill.tons > 0 && (
                <LineRow
                  label="Grade Fill"
                  value={fh(gradeFill.hours) || '—'}
                  sub={`${gradeFill.tons.toFixed(1)} tons  ·  ${rateBase} t/hr`}
                />
              )}
              {jjTons > 0 && (
                <LineRow
            ...
            <>
              ...
              {treeRows.map((r, i) => {
                ...
                return (
                  <LineRow
                    key={i}
                    label={`${r.size} Trees × ${r.qty} @ ${r.height}ft`}
                    value={fh(cr.hrs) || '—'}
          ...
          {!isSelf && subCost > 0 && (
            <>
              ...
              <LineRow
                label="Sub hauls all material"
                value={fmt(subCost)}
                sub="Based on SF × sub rates"
                highlight
              />
            </>
          )}
