...
export default function MiniSkidSteerDemoSummary({ module }) {
  ...
  const miscVertCalc = miscVertRows.map(r => vert(r.lf, r.heightIn || 0, r.widthIn || 8, rateConc, dumpConc))
  ...
  const hasLines = conc.tons || dirt.tons || base.tons || grass.tons ||
    ...
    footingCalc.some(r => r.tons) || gradeCut.tons || gradeFill.tons ||
    jjTons || ssCmpTons || n(shrubQty) || treeCalc.some(r => r.hrs) ||
        ...
        <>
          ...
          {(gradeCut.tons || gradeFill.tons || jjTons || ssCmpTons) > 0 && (
            <>
              ...
              {ssCmpTons      > 0 && <LineRow label='Mini SS Compact (4" Max)' value={fh(ssCmpHrs) || '—'}      sub={`${ssCmpTons.toFixed(1)} tons  ·  ${rateSSCmp} t/hr`} />}
            </>
          )}

          {/* Vegetation */}
          {(n(shrubQty) || n(stumpFirstQty) || n(stumpAddQty) || treeCalc.some(r => r.hrs > 0)) > 0 && (
            <>
