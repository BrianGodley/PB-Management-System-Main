...
// SHARED finish material — the Finishes module's own records (sub 'Finish Material'
// under category 'Finishes'), same as Fire Pit / Walls / Columns. One price per finish.
...
function calcOutdoorKitchen(
  ...
  ;[
    ...
    ;(rows || []).forEach(r => {
      const qty = n(r.qty)
      if (qty <= 0) return
      const { matCost, laborVal, laborName } = resolveUtilRow(cat, r, arr, materialRows, mp)
      if (laborVal <= 0) laborUnset.push({ kind: 'labor', name: laborName, label: r.type, category: 'Utilities', unit: null })
      epMat += qty * matCost
      epHrs += qty * laborVal
    })
...
export default function OutdoorKitchenModule({ onSave, onBack, saving, initialData }) {
  ...
  const vendorsForFinish = () => vendors.filter(v => materialRows.some(r => r.vendor_id === v.id && r.sub_category === WF_CAT && r.category === 'Finishes'))
