...
const OK_RATES = {
  // ── Material costs ──────────────────────────────────────────────────────────
  bbqBlock: { dbName: 'BBQ Block', fallback: 2.5 },
  bbqRebar: { dbName: 'BBQ Rebar', fallback: 0.4 },
  bbqConcrete: { dbName: 'BBQ Concrete', fallback: 149.5 },
  bbqFillMat: { dbName: 'BBQ Fill Material', fallback: 60.0 },
  applianceHardware: { dbName: 'BBQ Appliance Hardware', fallback: 3.0 },
  gficOutlet: { dbName: 'GFIC Outlet - BBQ', fallback: 80.0 },
  sinkPlumbing: { dbName: 'Sink Plumbing - BBQ', fallback: 115.0 },
  gasPipe: { dbName: 'Gas Pipe - BBQ', fallback: 3.0 },
  sandStucco: { dbName: 'Sand Stucco - BBQ', fallback: 0.0 },
  smoothStucco: { dbName: 'Smooth Stucco - BBQ', fallback: 0.0 },
  ledgerstone: { dbName: 'Ledgerstone - BBQ', fallback: 10.0 },
  stackedStone: { dbName: 'Stacked Stone - BBQ', fallback: 10.0 },
  tile: { dbName: 'Tile - BBQ', fallback: 6.5 },
  realFlagstone: { dbName: 'Real Flagstone - BBQ', fallback: 400.0 },
  realStone: { dbName: 'Real Stone - BBQ', fallback: 400.0 },
...
export default function OutdoorKitchenSummary({ module }) {
  ...
  if (totalLF > 0) {
    ...
    const excavHrs =
      (totalLF * footingAreaSF) / mp(OK_RATES.excavateLab.dbName, OK_RATES.excavateLab.fallback)
    ...
    const installHrs =
      (blockWaste / mp(OK_RATES.installBlockLab.dbName, OK_RATES.installBlockLab.fallback)) * 8
    const fillHrs =
      (((80 / 75) * blockRaw) / mp(OK_RATES.fillBlockLab.dbName, OK_RATES.fillBlockLab.fallback)) *
      8
  ...
  if (n(counterSF) > 0) {
    ...
    const formHrs =
      (n(counterSF) * 2) / mp(OK_RATES.counterFormLab.dbName, OK_RATES.counterFormLab.fallback)
    const pourHrs =
      (n(counterSF) / mp(OK_RATES.counterPourLab.dbName, OK_RATES.counterPourLab.fallback)) * 8
    const finishHrs =
      counterFinish === 'Broom Finish'
        ? (n(counterSF) / mp(OK_RATES.counterBroomLab.dbName, OK_RATES.counterBroomLab.fallback)) *
          8
        : (n(counterSF) /
            mp(OK_RATES.counterPolishLab.dbName, OK_RATES.counterPolishLab.fallback)) *
          8
    counterLines.push({
  ...
  const serviceLines = []
  if (n(layoutHrs) > 0)
    serviceLines.push({ label: 'Layout Time', value: '—', sub: `${n(layoutHrs).toFixed(1)} hrs` })
  if (n(applianceCount) > 0) {
    const mat =
      n(applianceCount) * mp(OK_RATES.applianceHardware.dbName, OK_RATES.applianceHardware.fallback)
    const hrs =
      (n(applianceCount) / mp(OK_RATES.applianceLab.dbName, OK_RATES.applianceLab.fallback)) * 8
    serviceLines.push({
      label: `Appliances/Openings — ${n(applianceCount)}`,
      value: fmt2(mat),
  ...
  if (n(gficCount) > 0) {
    ...
    serviceLines.push({
      label: `GFIC Outlets — ${n(gficCount)}`,
      value: fmt2(mat),
      sub: `${hrs.toFixed(0)} hrs`,
    })
  }
  if (sinkYN === 'Yes') {
  ...
  if (n(gasTrenchLF) > 0) {
    ...
    const hrs =
      (n(gasTrenchLF) / mp(OK_RATES.gasTrenchLab.dbName, OK_RATES.gasTrenchLab.fallback)) * 8
    serviceLines.push({
      label: `Gas Trench/Run — ${n(gasTrenchLF).toLocaleString()} LF`,
      value: fmt2(mat),
      sub: `${hrs.toFixed(2)} hrs`,
    })
  }
  ...
  if (n(sandStuccoSF) > 0) {
    ...
    const hrs =
      (n(sandStuccoSF) / mp(OK_RATES.sandStuccoLab.dbName, OK_RATES.sandStuccoLab.fallback)) * 8
    finishLines.push({
      label: `Sand Stucco — ${n(sandStuccoSF).toLocaleString()} SF`,
      value: fmt2(mat),
      sub: `${hrs.toFixed(2)} hrs`,
    })
  }
  if (n(smoothStuccoSF) > 0) {
    ...
    const hrs =
      (n(smoothStuccoSF) / mp(OK_RATES.smoothStuccoLab.dbName, OK_RATES.smoothStuccoLab.fallback)) *
      8
    finishLines.push({
      label: `Smooth Stucco — ${n(smoothStuccoSF).toLocaleString()} SF`,
      value: fmt2(mat),
      sub: `${hrs.toFixed(2)} hrs`,
    })
  }
  if (n(ledgerstoneSF) > 0) {
    ...
    const hrs =
      (n(ledgerstoneSF) / mp(OK_RATES.ledgerstoneLab.dbName, OK_RATES.ledgerstoneLab.fallback)) * 8
    finishLines.push({
      label: `Ledgerstone — ${n(ledgerstoneSF).toLocaleString()} SF`,
      value: fmt2(mat),
  ...
  if (n(stackedStoneSF) > 0) {
    ...
    const hrs =
      (n(stackedStoneSF) / mp(OK_RATES.stackedStoneLab.dbName, OK_RATES.stackedStoneLab.fallback)) *
      8
    finishLines.push({
      label: `Stacked Stone — ${n(stackedStoneSF).toLocaleString()} SF`,
      value: fmt2(mat),
      sub: `${hrs.toFixed(2)} hrs · ${fmt2(rate)}/SF`,
    })
  }
  if (n(tileSF) > 0) {
    ...
    finishLines.push({
      label: `Tile — ${n(tileSF).toLocaleString()} SF`,
      value: fmt2(mat),
      sub: `${hrs.toFixed(2)} hrs · ${fmt2(rate)}/SF`,
    })
  }
  if (n(flagstoneSF) > 0) {
    const rate =
      n(flagstoneRateInput) || mp(OK_RATES.realFlagstone.dbName, OK_RATES.realFlagstone.fallback)
    ...
    finishLines.push({
      label: `Real Flagstone — ${n(flagstoneSF).toLocaleString()} SF`,
      value: fmt2(mat),
  ...
  if (n(realStoneSF) > 0) {
    ...
    finishLines.push({
      label: `Real Stone — ${n(realStoneSF).toLocaleString()} SF`,
      value: fmt2(mat),
      sub: `${hrs.toFixed(2)} hrs · ${tons.toFixed(2)} tons · ${fmt2(rate)}/ton`,
    })
  }
