...
import { calcDemo } from './skidSteerCalc'
...
// One Main Demo "area" — Concrete / Soil / Grass / Grade-Cut + its own Rebar.
// Add Demo appends more of these; each is priced fully independently.
...
const DEFAULT_STATE = {
  ...
  // Main Demo — one or more independent areas (Concrete/Soil/Grass/Grade-Cut).
  mainDemoSections: [{ ...NEW_DEMO_SECTION }],
  // Demolition (legacy flat fields — kept for old estimates / sub tab)
  concSF: '',
  concDepth: 4,
  dirtSF: '',
  dirtDepth: 4,
...
export default function SkidSteerDemoModule({ initialData, onSave, onCancel, onSwitchType }) {
  const [state, setState] = useState(() => {
    ...
    // Old estimates predate Add Demo — fold their flat Main Demo fields into one
    // section so numbers reproduce exactly and the section UI has something to show.
    if (initialData && !Array.isArray(initialData.mainDemoSections)) {
      init.mainDemoSections = [
        {
          concSF: initialData.concSF ?? '',
          concDepth: initialData.concDepth ?? 4,
          dirtSF: initialData.dirtSF ?? '',
          dirtDepth: initialData.dirtDepth ?? 4,
          grassSF: initialData.grassSF ?? '',
          grassDepth: initialData.grassDepth ?? 4,
          gradeCutSF: initialData.gradeCutSF ?? '',
          gradeCutDepth: initialData.gradeCutDepth ?? 4,
  ...
  const dumpConc = n(materialPrices['Demo - Skid Dump - Concrete'])
  const dumpDirt = n(materialPrices['Demo - Skid Dump - Dirt'])
  const dumpGreen =
    n(materialPrices['Demo - Skid Dump - Green Waste'])
    ...
    <div className="space-y-4">
        ...
        {isSelf ? (
          <>
            {(state.mainDemoSections || []).map((sec, si) => {
              ...
              const rows = [
                { label: 'Concrete', key: 'conc', sfK: 'concSF', dK: 'concDepth', rateNote: `${calc.laborConc} hr/Cu Yd${sec.rebar ? ' ×rebar' : ''}` },
                { label: 'Dirt/Rock', key: 'dirt', sfK: 'dirtSF', dK: 'dirtDepth', rateNote: `${calc.laborDirt} hr/Cu Yd` },
                { label: 'Grass/Sod', key: 'grass', sfK: 'grassSF', dK: 'grassDepth', rateNote: `${calc.laborGrass} hr per 100 Sq Ft·in` },
                { label: 'Grade Cut', key: 'gradeCut', sfK: 'gradeCutSF', dK: 'gradeCutDepth', rateNote: `${calc.laborGradeCut} hr/Sq Ft` },
              ]
              return (
                <div key={si} className="mb-3 border border-gray-100 rounded-lg p-2">
                  ...
                  <div className="mt-2 flex items-center gap-3 px-1">
                    ...
                    {sec.rebar && <p className="text-xs text-gray-500">Concrete labor ×1.3</p>}
                  </div>
                </div>
              )
            })}
      ...
      <div>
        <div className="flex items-center gap-2 text-xs font-bold text-gray-600 uppercase tracking-wider bg-gray-50 rounded-lg border border-gray-200 px-4 py-2.5 mt-4 mb-2">
          {subSectionTitle('Compaction', isDemoSub)}
        </div>
        {isSelf && (
        <table className="w-full text-xs">
          <TH
      ...
      <div>
        ...
        <button
          type="button"
          onClick={() => set(isDemoSub ? 'subTreeRows' : 'treeRows', [...(isDemoSub ? state.subTreeRows : state.treeRows), { qty: '', height: 20, size: 'Small' }])}
          className="mt-2 text-xs px-2 py-1 rounded bg-slate-100 text-slate-700 hover:bg-slate-200"
        >
          + Add Row
        </button>
