...
test('PARITY (M5): install-tier mix selected by material ref_key — price resolves by key AND hand-mix uplift fires via the RESOLVED name (not the raw key)', () => {
  ...
  const mk = mixRef =>
    run(
      ...
      { lr: { 'LAB-076-concrete-install-300-600': 0.1, 'LAB-072-concrete-hand-mix-labor-uplift': 1.3 }, materialRows: [stdMix, handMix] }
    )
  ...
  // Hand-mix labor FACTOR (1.3 = ×1.3) applies via the resolved item name, not the stored key.
  assert.ok(Math.abs(hm.installHrs - std.installHrs * 1.3) < 0.001, `factor is 1.3: ${hm.installHrs} vs ${std.installHrs}×1.3`)
})

test('hand-mix factor unset → ×1 (never zeroes the labor)', () => {
  const handMix = { id: 'm2', ref_key: 'MAT-051-hand-mix', name: 'Hand Mix', sub_category: 'Concrete Mix', category: 'Basic Materials', vendor_id: null, unit_cost: 200, calc_meta: {} }
  // No LAB-072 rate provided → factor resolves to 1, so hand mix hours == the base tier hours.
  const r = run(
    { installTiers: { s300_600: 540 }, installTierVendor: { s300_600: 'Standard' }, installTierType: { s300_600: 'MAT-051-hand-mix' }, installTierDepth: { s300_600: 4 } },
    { lr: { 'LAB-076-concrete-install-300-600': 0.1 }, materialRows: [handMix] }
  )
  assert.ok(r.installHrs > 0, `install hours must not be zeroed by an unset factor; got ${r.installHrs}`)
