...
//
// Admin screen for the Formulas extension: view the standard (read-only) default
// conditions and their handling steps, customize a default's steps for this
// tenant, and create/edit/delete fully custom conditions.
//
// Tenants can't edit the global defaults (tenant_id IS NULL). "Customize" clones
// a default into a tenant-owned condition with the SAME slug (so the engine's
// computed condition picks up the tenant's steps), pre-filled with the default
// steps, which are then editable.
...
export default function ConditionManager() {
  ...
  const [editing, setEditing] = useState(null) // { id?, name, slug, isNew, fromDefault, steps:[{text}] }
  ...
  // A default (tenant_id null) is "overridden" if a tenant row with the same slug exists.
  ...
  function startCustomizeDefault(c) {
    setErr(null)
    const steps = (stepsByCond[c.id] || []).map(s => ({ text: s.text }))
    setEditing({ name: c.name, slug: c.slug, isNew: true, fromDefault: true, steps: steps.length ? steps : [{ text: '' }] })
  }
  ...
  const setStepText = (i, v) =>
    setEditing(e => ({ ...e, steps: e.steps.map((s, j) => (j === i ? { text: v } : s)) }))
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-4">
        <h2 className="text-sm font-bold text-gray-900">
          {editing.isNew ? (editing.fromDefault ? `Customize “${editing.name}” for your company` : 'New custom condition') : `Edit “${editing.name}”`}
        </h2>
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">
              Key {editing.fromDefault && <span className="text-gray-400">(matches a standard condition)</span>}
            </label>
            <input
              className={input}
              value={editing.slug}
              placeholder={slugify(editing.name)}
              disabled={editing.fromDefault}
        ...
        <div className="flex justify-end gap-2">
          <button onClick={() => setEditing(null)} className="btn-secondary text-sm px-4 py-2 border border-gray-200 rounded-lg">Cancel</button>
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-between items-center">
        <p className="text-sm text-gray-500">Standard conditions are read-only; customize their steps or add your own.</p>
      ...
      {loading ? (
        ...
        conditions.map(c => {
          ...
          if (overridden) return null // hide the default when the tenant has its own version
            ...
            <div key={c.id} className="bg-white border border-gray-200 rounded-xl p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex gap-2">
                  {isDefault ? (
                    <button onClick={() => startCustomizeDefault(c)} className="text-xs text-green-700 font-medium hover:text-green-900">Customize steps</button>
                  ) : (
                    <>
