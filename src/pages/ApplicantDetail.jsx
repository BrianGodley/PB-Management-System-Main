...
export default function ApplicantDetail() {
    ...
    setDraft(
      data
        ? {
            ...
            applicant_references: data.applicant_references || [],
          }
        : {}
    )
    setLoading(false)
  ...
  async function handleSave() {
    ...
    await supabase
      .from('applicants')
      .update({ ...draft, resume_url: resumeUrl })
      .eq('id', id)
    setSaving(false)
    setEditing(false)
    fetchApplicant()
  ...
  function updateWorkExp(idx, patch) {
    setDraft(prev => ({
      ...prev,
      work_experience: (prev.work_experience || []).map((e, i) =>
        i === idx ? { ...e, ...patch } : e
      ),
    }))
  }
  ...
  function updateRef(idx, patch) {
    setDraft(prev => ({
      ...prev,
      applicant_references: (prev.applicant_references || []).map((r, i) =>
        i === idx ? { ...r, ...patch } : r
      ),
    }))
  }
  function removeRef(idx) {
    setDraft(prev => ({
      ...prev,
      applicant_references: (prev.applicant_references || []).filter((_, i) => i !== idx),
    }))
  }
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
        <div className="max-w-3xl mx-auto space-y-5">
                ...
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {applicant.position_applied || 'No position specified'}
                    </p>
                  </div>
                </div>
                    ...
                    <button
                      onClick={() => {
                        ...
                        setDraft({
                          ...
                          applicant_references: applicant.applicant_references || [],
                        })
                      }}
                      className="px-3 py-1.5 border border-gray-200 rounded-lg text-xs font-medium text-gray-600 hover:bg-gray-50"
                    >
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <div key={key}>
                  ...
                  {editing ? (
                    ...
                    <p className="text-sm text-gray-800">
                      {applicant[key] || <span className="text-gray-400 italic">—</span>}
                    </p>
                  )}
                </div>
              ))}
              <div>
                ...
                {editing ? (
                  <input
                    value={draft.position_applied || ''}
                    onChange={e => set('position_applied', e.target.value)}
                    className="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-green-500"
                  />
                ) : (
                  <p className="text-sm text-gray-800">
                    {applicant.position_applied || <span className="text-gray-400 italic">—</span>}
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            {applicant.resume_url ? (
              <div className="flex items-center gap-3">
                ...
                {editing && <span className="text-xs text-gray-400">Upload below to replace</span>}
              </div>
            ) : (
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            {(editing ? draft.work_experience : applicant.work_experience || []).length === 0 ? (
              ...
              <div className="space-y-4">
                {(editing ? draft.work_experience : applicant.work_experience || []).map(
                  (exp, idx) => (
                    <div
                      key={exp.id || idx}
                      className={`${editing ? 'border border-gray-200 rounded-xl p-4' : 'border-l-2 border-gray-200 pl-4'}`}
                    >
                      {editing ? (
                        <div className="space-y-2">
                          <div className="flex gap-2">
                            ...
                            <input
                              value={exp.title}
                              onChange={e => updateWorkExp(idx, { title: e.target.value })}
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            {editing ? (
              ...
              <p className="text-sm text-gray-700 whitespace-pre-wrap">
                {applicant.skills || <span className="text-gray-400 italic">No skills listed</span>}
              </p>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            {(editing ? draft.applicant_references : applicant.applicant_references || [])
              .length === 0 ? (
              <p className="text-sm text-gray-400 italic">No applicant_references listed</p>
            ) : (
              <div className="space-y-3">
                {(editing ? draft.applicant_references : applicant.applicant_references || []).map(
                  (ref, idx) => (
                    <div
                      key={ref.id || idx}
                      className={`${editing ? 'flex gap-2 items-center' : 'flex items-center gap-4'}`}
                    >
                      {editing ? (
                        <>
                          <input
                        ...
                        <div className="flex-1">
                          ...
                          {ref.relation && (
                            <span className="text-gray-500 text-sm ml-2">({ref.relation})</span>
                          )}
                          {ref.phone && (
                            <span className="text-gray-500 text-sm ml-3">📞 {ref.phone}</span>
                          )}
                        </div>
                      )}
                    </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            {editing ? (
              ...
              <p className="text-sm text-gray-700 whitespace-pre-wrap">
                {applicant.notes || <span className="text-gray-400 italic">No notes</span>}
              </p>
            )}
          </div>
        </div>
