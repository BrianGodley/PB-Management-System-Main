...
export default function Bids() {
    ...
    try {
      ...
      if (!resolvedClientId && bid.client_name) {
        const { data: matches } = await supabase
          .from('clients')
          .select('id')
          .ilike('name', bid.client_name.trim())
          .limit(2)
  ...
  function getVal(bid, col) {
    switch (col) {
      case 'client_name':
        return (bid.client_name || '').toLowerCase()
      case 'last_name': {
      ...
      case 'estimate_name':
        return (bid.estimates?.estimate_name || '').toLowerCase()
      case 'created_at':
        return bid.created_at || ''
      case 'salesperson':
        return (bid.salesperson || '').toLowerCase()
      case 'gross_profit':
        return parseFloat(bid.gross_profit || 0)
      case 'gpmd':
        return parseFloat(bid.gpmd || 0)
      case 'bid_amount':
        return parseFloat(bid.bid_amount || 0)
      case 'status':
        return bid.status || ''
      default:
        return ''
  ...
  const totalBidValue = bids
    ...
    .reduce((s, b) => s + parseFloat(b.bid_amount || 0), 0)
  const soldValue = bids
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl mx-4 px-6 py-5">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Marking this bid as <span className="text-green-700 font-semibold">Sold</span>{' '}
            ...
            <div className="bg-gray-50 rounded-xl px-4 py-2.5 mb-3 text-sm text-gray-600 flex flex-wrap gap-x-5 gap-y-1">
              ...
              <p>
                ...
                {parseFloat(soldModal.bid?.bid_amount || 0).toLocaleString()}
              </p>
              {soldModal.bid?.projects?.length > 0 && (
                <p>
                  <span className="text-gray-400">Projects:</span>{' '}
                  {soldModal.bid.projects.join(', ')}
                </p>
              )}
            </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4 space-y-2 text-sm">
              ...
              {cascadeModal.woCount > 0 && (
                <p className="font-semibold text-red-600">
                  ⚠️ {cascadeModal.woCount} Work Order{cascadeModal.woCount !== 1 ? 's' : ''}{' '}
                  associated with this bid will also be deleted.
                </p>
              )}
            </div>
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-6 mt-4 flex-shrink-0">
            ...
            <div className="card text-center px-2 py-3 sm:px-4 sm:py-4 min-w-0">
              ...
              <p className="font-bold text-green-700 text-sm sm:text-lg leading-tight break-words">
                ${soldValue.toLocaleString()}
              </p>
            </div>
            <div className="card text-center px-2 py-3 sm:px-4 sm:py-4 min-w-0">
              <p className="text-[10px] sm:text-xs text-gray-500 mb-1 truncate">Close Rate</p>
              <p className="font-bold text-blue-700 text-sm sm:text-lg leading-tight">
                {closeRate.toFixed(0)}%
              </p>
            </div>
          </div>
                      ...
                      >
                        <td className="px-4 py-3">
                          ...
                          {bid.estimates?.client_id ? (
                            <Link
                              to={`/clients/${bid.estimates.client_id}`}
                          ...
                          {bid.estimates?.estimate_name && (
                            <p className="text-xs text-gray-500">{bid.estimates.estimate_name}</p>
                          )}
                          {bid.job_address && (
                          ...
                          {bid.salesperson && (
                            <p className="text-xs text-gray-400">👤 {bid.salesperson}</p>
                          )}
                          {bid.estimate_id && (
                            <Link
                          ...
                          {bid.projects && bid.projects.length > 0 ? (
                            <div className="flex flex-wrap gap-1" style={{ width: '256px' }}>
                              {bid.projects.map((proj, j) => (
                                <span
                                  key={j}
                                  className="text-xs px-2 py-0.5 bg-green-50 text-green-800 border border-green-200 rounded-full break-words"
                                >
                                  {proj}
                                </span>
                              ))}
                        ...
                        <td className="px-4 py-3 text-right whitespace-nowrap text-gray-700">
                          {bid.gpmd > 0 ? (
                            `$${Math.round(bid.gpmd).toLocaleString()}`
                          ) : (
    ...
    <div className="w-full">
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        <p className="text-sm text-gray-500 mb-5">
          Sets the percentage added to the cash price for the{' '}
          <em>"Job Total $X - Finance OAC Price"</em> line on every bid document. The Sauer template
          default is 10%.
        </p>
              ...
              return (
                <div className="mt-5 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                  ...
                  <p className="text-sm text-gray-700">
                    Cash/Check price{' '}
                    <span className="font-semibold">${example.toLocaleString()}</span> &nbsp;→&nbsp;
                    Finance OAC price{' '}
                    <span className="font-semibold text-green-800">
                      ${finance.toLocaleString()}
                    </span>
