  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <input
          type="text" placeholder="Search bids..." value={search}
          ...
          {['all', ...BID_STATUSES].map(s => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium capitalize transition-colors ${
                filter === s
                  ? (STATUS_FILTER[s] || 'bg-green-700 text-white')
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      ...
                      {bid.estimate_id && (
                        <Link to={`/estimates/${bid.estimate_id}`} className="text-xs text-green-700 hover:underline">
                          📋 View Estimate
                        </Link>
                      )}
                    </td>
                      ...
                      {bid.projects && bid.projects.length > 0 ? (
                        <div className="flex flex-wrap gap-1">
                          {bid.projects.map((proj, j) => (
                            <span key={j} className="text-xs px-2 py-0.5 bg-green-50 text-green-800 border border-green-200 rounded-full">
                              {proj}
                            </span>
                          ))}
                        </div>
