...
export default function ClientDetail() {
  ...
  async function fetchData() {
    ...
    if (clientData) {
      ...
      const { data: jobsData } = await supabase
        .from('jobs').select('*').eq('client_name', clientData.name).order('sold_date', { ascending: false })
      ...
      if (jobsData) {
        ...
        if (jobsData.length > 0) {
          const { data: coData } = await supabase
            ...
            .in('linked_job_id', jobsData.map(j => j.id))
            .order('date_submitted', { ascending: false })
          if (coData) {
        ...
        if (estData) setEstimates(estData.filter(e => !coEstimateIds.has(e.id) && (e.estimate_name || '').trim() !== ''))
      } else {
  ...
  async function handleEstimateNext(data) {
    const { data: est, error } = await supabase.from('estimates').insert({
      estimate_name: data.name, type: data.type,
      client_name: client.name, client_id: client.id, status: 'pending',
    }).select().single()
    if (error) { alert(`Error: ${error.message}`); return }
    if (est) { setShowEstimateModal(false); navigate(`/estimates/${est.id}`) }
  }
  ...
  async function handleDelete() {
    ...
    setDeleteModal({
      estCount: ests?.length || 0, bidCount: bidsData?.length || 0, woCount,
      onConfirm: async () => {
  ...
  function sumModules(est, field) {
    return (est.estimate_projects || []).reduce((s, proj) =>
      s + (proj.estimate_modules || []).reduce((ms, mod) => ms + parseFloat(mod[field] || 0), 0), 0)
  }
  ...
  return (
    <div className="flex flex-col h-full">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6">
            ...
            <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-4 text-sm space-y-1.5">
              <p className="text-gray-700 font-medium">
                Deleting <span className="font-bold text-gray-900">{displayName(client) || client.name}</span> will permanently remove:
              </p>
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteModal(null)} className="flex-1 px-4 py-2 rounded-lg border border-gray-300 text-gray-600 font-semibold hover:bg-gray-50">Cancel</button>
              <button onClick={deleteModal.onConfirm} className="flex-1 bg-red-600 text-white font-semibold py-2 rounded-lg hover:bg-red-700">Delete Everything</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="h-full overflow-hidden grid" style={{ gridTemplateColumns: '20rem 1fr' }}>
            ...
            <div className="border-r border-slate-300 bg-slate-200 overflow-y-auto">
              <div className="p-3 space-y-2">
                    ...
                    <form onSubmit={handleSave} className="space-y-2.5">
                      ...
                      {[
                        ...
                        <div key={f.key}>
                          ...
                          <input type={f.type} className="w-full border border-slate-300 rounded-lg px-2.5 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-600/30 focus:border-green-600"
                      ...
                      <div className="flex gap-2 pt-1">
                        <button type="submit" disabled={saving} className="flex-1 py-1.5 rounded-lg bg-green-700 text-white text-xs font-semibold hover:bg-green-800 disabled:opacity-50">
                          {saving ? 'Saving…' : 'Save'}
                        </button>
                        <button type="button" onClick={() => setEditing(false)} className="flex-1 py-1.5 rounded-lg border border-gray-200 text-xs font-medium text-gray-600 hover:bg-gray-50">Cancel</button>
                      </div>
                    </form>
                  ) : (
                    <>
                      ...
                      <div className="space-y-1.5 text-xs">
                        {(client.street || client.city) && (
                          <div className="flex items-start gap-1.5 text-gray-700">
                            ...
                            <span className="font-semibold">{[client.street, [client.city, client.state, client.zip].filter(Boolean).join(' ')].filter(Boolean).join(', ')}</span>
                          </div>
                        )}
                        {client.phone && (
                        ...
                        {client.notes && (
                          <div className="pt-2 border-t border-slate-100">
                            ...
                            <p className="font-semibold text-gray-700 leading-relaxed">{client.notes}</p>
                          </div>
                        )}
                      </div>
                    </>
                ...
                {/* Stats card */}
                <div className="bg-white border border-slate-300 rounded-xl p-4 shadow-sm">
                  <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide mb-2.5">Summary</p>
                  <div className="grid grid-cols-4 gap-1.5 mb-3">
                    {[
                      { label: 'Ests',  count: estimates.length, color: 'bg-gray-50   text-gray-700  border-gray-200' },
                      { label: 'Bids',  count: bids.length,      color: 'bg-blue-50   text-blue-700  border-blue-200' },
                      { label: 'Jobs',  count: soldJobs.length,  color: 'bg-green-50  text-green-800 border-green-200' },
                      { label: 'C/Os',  count: totalCOs,         color: 'bg-orange-50 text-orange-700 border-orange-200' },
                    ].map(s => (
                  ...
                  <div className="space-y-1.5 border-t border-slate-100 pt-2.5">
                    {[
                      ...
                      <div key={r.label} className="flex items-center justify-between text-xs">
                        ...
                        <span className={`font-bold ${r.highlight ? 'text-green-700' : 'text-gray-900'}`}>{r.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="p-3 space-y-3">
                ...
                <div className="bg-white border border-slate-300 rounded-xl shadow-sm overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-500 font-semibold">{estimates.length}</span>
                    </div>
                    <button onClick={() => setShowEstimateModal(true)} className="text-xs px-3 py-1.5 rounded-lg bg-green-700 text-white font-semibold hover:bg-green-800 transition-colors">
                      + New Estimate
                    </button>
                  </div>
                  {estimates.length === 0 ? (
                ...
                <div className="bg-white border border-slate-300 rounded-xl shadow-sm overflow-hidden">
                  <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-200">
                    ...
                    <span className="text-xs px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-500 font-semibold">{bids.length}</span>
                  </div>
                  {bids.length === 0 ? (
                            ...
                            return (
                              <tr key={bid.id} className="hover:bg-gray-50 transition-colors">
                                <td className="px-4 py-2">
                                  {bid.estimate_id
                                    ? <Link to={`/estimates/${bid.estimate_id}`} className="font-semibold text-green-700 hover:underline">{bid.estimates?.estimate_name || bid.client_name}</Link>
                                    : <p className="font-semibold text-gray-800">{bid.estimates?.estimate_name || bid.client_name}</p>}
                                  {bid.projects?.length > 0 && <p className="text-[10px] text-gray-400 mt-0.5">{bid.projects.join(', ')}</p>}
                                </td>
                                ...
                                <td className="px-3 py-2 text-left text-gray-600 whitespace-nowrap">
                                  {bid.estimates?.created_by ? (profiles[bid.estimates.created_by] || '—') : '—'}
                                </td>
                                <td className="px-3 py-2 text-center">
                                  <button onClick={() => setViewingBid(bid)} disabled={!bid.estimate_id}
                                ...
                                <td className="px-3 py-2 text-center">
                                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${BID_STATUS_STYLES[status] || BID_STATUS_STYLES.pending}`}>
                  ...
                  {soldJobs.length === 0 ? (
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-xs min-w-[560px]">
                        <thead>
                          <tr className="bg-gray-50 border-b border-gray-100 text-gray-500 uppercase tracking-wide">
                            <th className="px-4 py-2 text-left font-semibold">Job / Change Order</th>
                            <th className="px-3 py-2 text-right font-semibold">Date</th>
                            <th className="px-3 py-2 text-right font-semibold">Man Days</th>
                            <th className="px-3 py-2 text-right font-semibold">Materials</th>
                            <th className="px-3 py-2 text-right font-semibold">Gross Profit</th>
                            ...
                            return (
                              <>
                                <tr key={job.id} className="hover:bg-gray-50 transition-colors">
                                  ...
                                  <td className="px-3 py-2 text-right text-gray-700">{parseFloat(job.total_man_days || 0).toFixed(1)}</td>
                                  <td className="px-3 py-2 text-right text-gray-700">{fmt(job.material_cost)}</td>
                                  <td className="px-3 py-2 text-right font-semibold text-green-700">{fmt(job.gross_profit)}</td>
                                  <td className="px-3 py-2 text-right font-bold text-gray-900">{fmt(job.total_price)}</td>
                                  <td className="px-3 py-2 text-center">
                                ...
                                {cos.map(co => (
                                  <tr key={co.id} className="bg-blue-50/40 hover:bg-blue-50 border-b border-blue-100/60 transition-colors">
                                    <td className="pl-8 pr-3 py-1.5">
                                      <div className="flex items-center gap-1.5">
                                        ...
                                        {co.co_type && <span className="text-[9px] text-blue-400">{co.co_type}</span>}
                                      </div>
                                    </td>
                                    ...
                                    <td className="px-3 py-1.5 text-center">
                                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${BID_STATUS_STYLES[co.status || 'pending'] || BID_STATUS_STYLES.pending}`}>
                                        {(co.status || 'pending').charAt(0).toUpperCase() + (co.status || 'pending').slice(1)}
                                      </span>
                                    </td>
                                  </tr>
                                ))}
                        ...
                        {soldJobs.length > 1 && (
                          <tfoot className="border-t-2 border-gray-200 bg-gray-50">
                            <tr className="text-xs font-semibold text-gray-600">
                              ...
                              <td className="px-3 py-2 text-right">{soldJobs.reduce((s, j) => s + parseFloat(j.total_man_days || 0), 0).toFixed(1)}</td>
                              <td className="px-3 py-2 text-right">{fmt(soldJobs.reduce((s, j) => s + parseFloat(j.material_cost || 0), 0))}</td>
      ...
      {showEstimateModal && (
        <NewEstimateModal client={client} onClose={() => setShowEstimateModal(false)} onNext={handleEstimateNext} />
      )}
      {viewingBid && (
        <BidDocViewerModal bid={viewingBid} onClose={() => setViewingBid(null)} />
      )}
    </div>
  )
}
