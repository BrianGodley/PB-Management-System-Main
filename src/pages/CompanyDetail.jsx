                  ...
                  <div key={i} className="bg-white border border-gray-200 rounded-lg p-3 space-y-2">
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <input
                          ...
                          value={c.first_name || ''}
                          onChange={e =>
                            updateCompanyContact(i, { ...c, first_name: e.target.value })
                          }
                          placeholder="First"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          value={c.last_name || ''}
                          onChange={e =>
                            updateCompanyContact(i, { ...c, last_name: e.target.value })
                          }
                          placeholder="Last"
                        />
                      </div>
                    ...
                    <div>
                      ...
                      <input
                        className={inp}
                        value={c.position || ''}
                        onChange={e => updateCompanyContact(i, { ...c, position: e.target.value })}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <input
                          className={inp}
                          value={c.phone || ''}
                          onChange={e => updateCompanyContact(i, { ...c, phone: e.target.value })}
                          placeholder="(555) 000-0000"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          className={inp}
                          value={c.email || ''}
                          onChange={e => updateCompanyContact(i, { ...c, email: e.target.value })}
                          placeholder="email@example.com"
                        />
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() => removeCompanyContact(i)}
...
export default function CompanyDetail() {
  ...
  async function handleAddAsClient() {
    ...
    const { error } = await supabase.from('clients').insert({
      ...
      notes: c.notes || null,
      company_contacts:
        Array.isArray(c.company_contacts) && c.company_contacts.length ? c.company_contacts : null,
      created_by: user?.id,
    })
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-6 py-3 bg-white border-b border-gray-200 flex-shrink-0">
        ...
        <span className="text-gray-300">/</span>
        <span className="text-sm font-semibold text-gray-900 bg-gray-100 px-2 py-0.5 rounded-md truncate">
            ...
            <div className="border-r border-slate-300 bg-slate-200 overflow-y-auto">
              <div className="p-3 space-y-2">
                      ...
                      <div>
                        ...
                        {company.website && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="text-xs text-green-700 hover:underline truncate block max-w-[180px]"
                          >
                            {company.website}
                          </a>
                        )}
                    ...
                    <div className="space-y-3 text-sm">
                      <div>
                        ...
                        <p className="font-semibold text-gray-900">
                          {company.ghl_assigned_to || <span className="text-gray-300">—</span>}
                        </p>
                      </div>
                      <div>
                        <p className={lbl}>Contact Type</p>
                        <p className="font-semibold text-gray-900">
                          {company.contact_type || <span className="text-gray-300">—</span>}
                        </p>
                      </div>
                      <div>
                    ...
                    <div>
                      {contacts.length === 0 ? (
                        <p className="text-xs text-gray-400 italic text-center py-6">
                          No contacts on file — click edit to add.
                        </p>
                      ) : (
                        <div className="space-y-3">
                          {contacts.map((c, i) => (
                            <div
                              key={i}
                              className="bg-slate-50 border border-slate-300 rounded-xl p-3 space-y-1"
                            >
                              <p className="text-sm font-semibold text-gray-900">
                                {[c.first_name, c.last_name].filter(Boolean).join(' ') || '—'}
                              </p>
                              {c.position && (
                                <p className="text-xs text-green-700 font-medium">{c.position}</p>
                              )}
                              {c.phone && (
                                <a
                                  href={`tel:${c.phone}`}
                                  className="block text-xs font-semibold text-gray-900 hover:text-green-700"
                                >
                                  {c.phone}
                                </a>
                              )}
                              {c.email && (
                                <a
                                  href={`mailto:${c.email}`}
                                  className="block text-xs font-semibold text-gray-900 hover:text-green-700 break-all"
                                >
                                  {c.email}
                                </a>
                              )}
                  ...
                  {leftTab === 'marketing' && (
                    <div className="space-y-3 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-gray-900">
                          {company.source || <span className="text-gray-300">—</span>}
                        </p>
                      </div>
                      <div>
                        <p className={lbl}>Campaign</p>
                        <p className="font-semibold text-gray-900">
                          {company.campaign || <span className="text-gray-300">—</span>}
                        </p>
                      </div>
                      <div>
                        <p className={lbl}>Source Origin</p>
                        <p className="font-semibold text-gray-900">
                          {company.how_did_you_hear || <span className="text-gray-300">—</span>}
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {leftTab === 'tags' && (
                    <div>
                      {!company.tags || company.tags.length === 0 ? (
                        <p className="text-xs text-gray-400 italic text-center py-6">
                          No tags on file — click edit to add.
                        </p>
                      ) : (
                        <div className="flex flex-wrap gap-2">
                          {company.tags.map((tag, i) => (
                            <span
                              key={i}
                              className="inline-flex items-center px-2.5 py-1 rounded-full bg-green-50 text-green-800 text-xs font-medium border border-green-200"
                            >
                              {tag}
                            </span>
                          ))}
                ...
                <div className="bg-white border border-slate-300 rounded-xl p-4 shadow-sm space-y-3">
                  ...
                  {company.notes && (
                    <div
                      className={company.call_center_notes ? 'pt-2 border-t border-slate-100' : ''}
                    >
                  ...
                  <div
                    className={`text-xs text-gray-400 space-y-0.5 ${company.call_center_notes || company.notes ? 'pt-2 border-t border-slate-100' : ''}`}
                  >
                  ...
                  {comms.map(comm => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <div
                            className={`rounded-2xl px-4 py-2.5 text-sm shadow-sm ${isOut ? 'bg-green-700 text-white rounded-tr-sm' : 'bg-white text-gray-800 border border-slate-300 rounded-tl-sm'}`}
                          >
                ...
                <div className="bg-white border border-slate-300 rounded-xl p-3 shadow-sm">
                  ...
                  <div className="space-y-1">
                    {STAGES.map((s, i) => {
                      ...
                      const isPast =
                        i < stageOrder && s.value !== 'lost' && company.stage !== 'lost'
                      return (
                        <button
                          key={s.value}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  This will move the record to Individuals
                </p>
              </div>
            </div>
            <p className="text-sm text-gray-600 mb-5">
              <span className="font-semibold text-gray-800">{company.company_name}</span> will
              become an Individual contact. All communication history will be carried over. You can
              split the name into first/last after converting.
            </p>
            <div className="flex gap-3">
              ...
              <button
                onClick={handleMoveToIndividual}
                disabled={moving}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6">
            ...
            <p className="text-sm text-gray-600 mb-5">
              ...
              <span className="font-semibold text-gray-800">{company.company_name}</span>? All
              communication history will also be permanently removed.
            </p>
            <div className="flex gap-3">
              <button
