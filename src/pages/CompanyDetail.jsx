      ...
      <div className="flex-1 min-h-0 overflow-hidden grid" style={{gridTemplateColumns: '23rem minmax(0,1fr) 15rem'}}>
        ...
        <div className="border-r border-slate-200 bg-slate-100 overflow-y-auto">
          <div className="p-3 space-y-2">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-xl bg-green-700 flex items-center justify-center text-green-100 font-bold text-lg flex-shrink-0 select-none">
                  ...
                  <div>
                    ...
                    {company.website && <a href={company.website} target="_blank" rel="noreferrer" className="text-xs text-green-700 hover:underline truncate block max-w-[180px]">{company.website}</a>}
                  </div>
                </div>
              ...
              <div className="space-y-2 text-sm">
                <div>
                  ...
                  {addressLine ? <p className="font-semibold text-gray-900">{addressLine}</p> : <span className="text-gray-300">—</span>}
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <p className={lbl}>Main Phone</p>
                    {company.phone ? <a href={`tel:${company.phone}`} className="font-semibold text-gray-900 hover:text-green-700">{company.phone}</a> : <span className="text-gray-300">—</span>}
                  </div>
                  <div>
                    <p className={lbl}>Main Email</p>
                    {company.email ? <a href={`mailto:${company.email}`} className="font-semibold text-gray-900 hover:text-green-700 break-all">{company.email}</a> : <span className="text-gray-300">—</span>}
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Tab bar — sits on slate-100 background */}
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
              ...
              {leftTab === 'main' && (
                <div className="space-y-3 text-sm">
                  <div><p className={lbl}>Assigned To</p><p className="font-semibold text-gray-900">{company.ghl_assigned_to || <span className="text-gray-300">—</span>}</p></div>
                  <div><p className={lbl}>Contact Type</p><p className="font-semibold text-gray-900">{company.contact_type || <span className="text-gray-300">—</span>}</p></div>
                  <div>
                    <p className={lbl}>Project Description</p>
                    {company.project_description ? <p className="font-semibold text-gray-900 text-xs leading-relaxed whitespace-pre-wrap">{company.project_description}</p> : <span className="text-gray-300">—</span>}
                  </div>
                </div>
              )}
                    ...
                    <div className="space-y-3">
                      {contacts.map((c, i) => (
                        <div key={i} className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-1">
                          <p className="text-sm font-semibold text-gray-900">{[c.first_name, c.last_name].filter(Boolean).join(' ') || '—'}</p>
              ...
              {leftTab === 'marketing' && (
                <div className="space-y-3 text-sm">
                  <div>
                    ...
                    <select value={company.contact_type || 'Residential'} onChange={e => handleContactTypeChange(e.target.value)}
                  ...
                  <div><p className={lbl}>Source Type</p><p className="font-semibold text-gray-900">{company.source || <span className="text-gray-300">—</span>}</p></div>
                  <div><p className={lbl}>Campaign</p><p className="font-semibold text-gray-900">{company.campaign || <span className="text-gray-300">—</span>}</p></div>
                  <div><p className={lbl}>Source Origin</p><p className="font-semibold text-gray-900">{company.how_did_you_hear || <span className="text-gray-300">—</span>}</p></div>
                </div>
              )}
                  ...
                  {(!company.tags || company.tags.length === 0) ? (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {company.tags.map((tag, i) => (
                        <span key={i} className="inline-flex items-center px-2.5 py-1 rounded-full bg-green-50 text-green-800 text-xs font-medium border border-green-200">{tag}</span>
                      ))}
                    </div>
                  )}
                </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm space-y-3">
              ...
              {company.notes && (
                <div className={company.call_center_notes ? 'pt-2 border-t border-slate-100' : ''}>
              ...
              <div className={`text-xs text-gray-400 space-y-0.5 ${(company.call_center_notes || company.notes) ? 'pt-2 border-t border-slate-100' : ''}`}>
            ...
            {comms.map(comm => {
              ...
              return (
                <div key={comm.id} className={`flex gap-3 ${isOut ? 'flex-row-reverse' : ''}`}>
                  <div className="w-7 h-7 rounded-full bg-white border border-slate-200 flex items-center justify-center text-sm flex-shrink-0">{t.icon}</div>
                  <div className={`max-w-[75%] ${isOut ? 'items-end' : 'items-start'} flex flex-col`}>
                    <div className={`rounded-2xl px-4 py-2.5 text-sm shadow-sm ${isOut ? 'bg-green-700 text-white rounded-tr-sm' : 'bg-white text-gray-800 border border-slate-200 rounded-tl-sm'}`}>
