...
export default function ContactDetail() {
  ...
  async function handleAddAsClient() {
    ...
    const { data: newClient, error } = await supabase.from('clients').insert({
      ...
      phone:             c.phone                     || null,
      cell:              c.cell                      || null,
      additional_emails: Array.isArray(c.additional_emails) && c.additional_emails.length ? c.additional_emails : null,
      additional_phones: Array.isArray(c.additional_phones) && c.additional_phones.length ? c.additional_phones : null,
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-6 py-3 bg-white border-b border-gray-200 flex-shrink-0">
        ...
        <span className="text-gray-300">/</span>
        <span className="text-sm font-semibold text-gray-900 bg-gray-100 px-2 py-0.5 rounded-md">{fullName || 'Unnamed'}</span>
      </div>
