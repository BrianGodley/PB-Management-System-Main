...
export default function Design() {
  ...
  async function fetchAll() {
    ...
    const [{ data: projData }, { data: cliData }] = await Promise.all([
      ...
      supabase.from('clients').select('id, name, first_name, last_name').order('name'),
    ])
  ...
  const filtered = useMemo(() => {
    ...
    return projects.filter(
      p => p.name.toLowerCase().includes(q) || (p.clients?.name || '').toLowerCase().includes(q)
    )
  }, [projects, search])
      ...
      {activeTab === 'takeoffs' && (
        <>
          ...
          <div className="mb-6 p-3 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-800">
            🚧 <strong>Phase 1 of 5.</strong> Project list is live. File upload, viewer, scale
            calibration, drawing tools, and takeoff reports are coming in subsequent updates.
          </div>
              ...
              {search ? (
                ...
                <>
                  ...
                  <p className="text-sm mt-1">
                    Click <strong>+ New Project</strong> above to start your first takeoff.
                  </p>
                </>
              )}
            </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {filtered.map(p => (
                    <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-gray-500 text-xs truncate max-w-md">
                        {p.notes || '—'}
                      </td>
