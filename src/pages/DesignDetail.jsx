...
export default function DesignDetail() {
  ...
  function onPdfLoad({ numPages: n }) {
    ...
    const f = files.find(x => x.id === selectedFileId)
    if (f && f.num_pages !== n)
      supabase.from('design_files').update({ num_pages: n }).eq('id', f.id)
  }
  ...
  function onSvgClick(e) {
    ...
    if (tool === 'count') {
      ...
      if (!item) {
        setError('Pick or create a count item first.')
        return
      }
      saveAnnotation({
        type: 'count',
    ...
    if (tool === 'linear' || tool === 'area') {
      const item = items.find(it => it.id === activeItemId)
      if (!item) {
        setError('Pick or create an item first.')
        return
      }
  ...
  async function updateAnnotation(annId, patch) {
    const { error: upErr } = await supabase.from('design_annotations').update(patch).eq('id', annId)
    if (upErr) {
  ...
  async function createItem(payload) {
    const { data, error: insErr } = await supabase
      ...
      .single()
    if (insErr) {
      setError('Could not create item: ' + insErr.message)
      return null
    }
  ...
  async function applyScale() {
    ...
    const targetPages =
      scaleDialog.applyAll && numPages > 1
  ...
  if (!project)
    return (
      <div className="p-6 max-w-3xl mx-auto">
        <Link to="/design" className="text-sm text-green-700 hover:underline">
          ← Back to Design
        </Link>
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-2.75rem)] -m-4 lg:-m-6 bg-gray-100 overflow-hidden">
      {/* No top tab bar here — Design's top-level CAD / Selections /
          Take Offs tabs live on the Design index page. Landing on a project
          drops the user straight into the Take Off workspace. */}
      <div ref={containerRef} className="flex flex-1 overflow-hidden">
        ...
        >
          <div className="px-4 py-3 border-b border-gray-200 flex-shrink-0">
            <Link to="/design" className="text-xs text-green-700 hover:underline block mb-1">
              ← All Projects
            </Link>
            <h1 className="text-base font-bold text-gray-900 truncate" title={project.name}>
              {project.name}
            </h1>
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="px-2 py-2 border-b border-gray-100">
                ...
                {items.length === 0 ? (
                  <p className="px-2 py-3 text-xs text-gray-400">
                    Pick a tool to create your first named item.
                  </p>
                ) : (
                  items.map(it => {
                    ...
                    const total =
                      it.type === 'linear'
                        ? fmtLen(
                            itemAnns.reduce((s, a) => s + polylineLengthPx(a.points), 0),
                            pageScale?.ppu,
                            pageScale?.unit || ''
                          )
                        : it.type === 'area'
                          ? fmtArea(
                              itemAnns.reduce((s, a) => s + polygonAreaPx(a.points), 0),
                              pageScale?.ppu,
                              pageScale?.unit || ''
                            )
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          />
                          <span className="text-[10px] flex-shrink-0 w-3 text-center">
                            {TOOL_META[it.type]?.icon}
                          </span>
                          <span className="text-xs font-semibold flex-1 truncate" title={it.name}>
                            {it.name}
                          </span>
                          <span className="text-[10px] text-gray-500 flex-shrink-0">
                            {itemAnns.length} · {total}
                          </span>
                          <button
                            onClick={e => {
                              e.stopPropagation()
                              setEditingItemId(isExpanded ? null : it.id)
                            }}
                        ...
                        {isExpanded && (
                          <div className="px-3 pb-2 pt-1 space-y-1">
                            ...
                            <p className="text-[10px] text-gray-400">
                              {itemAnns.length} measurement{itemAnns.length === 1 ? '' : 's'}{' '}
                              attached.
                            </p>
                          </div>
                        )}
              ...
              {annotations.filter(a => !a.item_id && a.type !== 'scale').length > 0 && (
                <div className="px-4 py-2 border-b border-gray-100">
                  ...
                  {annotations
                    .filter(a => !a.item_id && a.type !== 'scale')
                    .map(a => {
                      const valueText =
                        a.type === 'linear'
                          ? fmtLen(
                              polylineLengthPx(a.points),
                              pageScale?.ppu,
                              pageScale?.unit || ''
                            )
                          : a.type === 'area'
                            ? fmtArea(
                                polygonAreaPx(a.points),
                                pageScale?.ppu,
                                pageScale?.unit || ''
                              )
                            : ''
                      return (
                        <div key={a.id} className="flex items-center gap-2 py-1 group">
                          ...
                          <button
                            ...
                            className="text-gray-300 group-hover:text-red-500 hover:!text-red-700 text-xs px-1"
                          >
                            ✕
                          </button>
                        </div>
          ...
          <div className="px-4 py-2 border-t border-gray-200 text-[10px] text-gray-400 flex-shrink-0">
            Phase 4 of 5 · report coming next
          </div>
        </aside>
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-2 px-4 py-2 bg-white border-b border-gray-200 flex-shrink-0 flex-wrap">
              ...
              <div className="flex items-center gap-1 ml-2">
                {Object.entries(TOOL_META).map(([k, m]) => (
                  <button
                    ...
                    title={
                      k === 'scale' && !pageScale
                        ? 'Set scale (required for measurements)'
                        : m.label
                    }
            ...
            {isPdfFile(selectedFile) && numPages > 0 && (
              <div className="flex items-center gap-1 mr-2">
                <button
                  ...
                  className="px-2 py-1 rounded text-sm border border-gray-300 hover:bg-gray-50 disabled:opacity-30"
                >
                  ‹
                </button>
            ...
            <div className="flex items-center gap-1">
              <button
                onClick={zoomOut}
                className="px-2 py-1 rounded text-sm border border-gray-300 hover:bg-gray-50"
                title="Zoom out"
              >
                −
              </button>
              <button
                onClick={zoomReset}
                className="px-2 py-1 rounded text-xs border border-gray-300 hover:bg-gray-50 min-w-[3.25rem]"
                title="Reset zoom"
              >
                {Math.round(zoom * 100)}%
              </button>
            ...
            <div className="bg-amber-50 border-b border-amber-200 px-4 py-1.5 text-xs text-amber-800 flex-shrink-0">
              {tool === 'scale' &&
                (drawing
                  ...
                  : 'Click the first point of a known distance, then click the second.')}
              {tool === 'linear' &&
                (drawing
              ...
              {tool === 'count' &&
                'Click anywhere to drop a count marker. Click an item in the sidebar to switch the active count.'}
            </div>
          )}
          ...
          >
            ...
            >
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-green-200 mt-0.5">
                    Name your measurement first, then draw on the page.
                  </p>
                </div>
                <button
                  onClick={() => setItemModal(null)}
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onKeyDown={e => {
                      if (e.key === 'Enter' && itemModal.name.trim()) confirmItemModal()
                    }}
                  />
                </div>
                ...
                <p className="text-xs text-gray-500 pt-1">
                  ...
                  {(itemModal.type === 'linear' || itemModal.type === 'area') &&
                    ' Double-click to finish a shape.'}{' '}
              ...
              <div className="px-6 py-3 border-t border-gray-200 bg-gray-50 flex justify-end gap-2">
                <button
                  onClick={() => setItemModal(null)}
                  className="px-4 py-2 rounded-lg text-sm text-gray-600 hover:bg-white border border-gray-300"
                >
                  Cancel
                </button>
                <button
              ...
              {scaleDialog.mode === 'page' && (
                <div className="p-6 space-y-3">
                  ...
                  <div>
                    ...
                    >
                      {(scaleDialog.measurementType === 'imperial'
                        ? IMPERIAL_SCALES
                        : METRIC_SCALES
                      ).map(s => (
                  ...
                  {numPages > 1 && (
                    <label className="flex items-center gap-2 cursor-pointer pt-1">
                      <input
                        ...
                        onChange={e =>
                          setScaleDialog({ ...scaleDialog, applyAll: e.target.checked })
                        }
                        className="w-4 h-4 rounded accent-green-700"
                      />
                      <span className="text-sm text-gray-700">
                        Apply same scale to all {numPages} pages
                      </span>
                    </label>
                  )}

                  <p className="text-xs text-gray-500 pt-1">
                    Picks pixels-per-unit from the standard scale assuming the drawing renders at 72
                    DPI (PDF default).
                  </p>
                </div>
              )}
                  ...
                  {!scaleDialog.points ? (
                    <>
                      <p className="text-sm text-gray-700">
                        Click <strong>Click 2 Points on Drawing</strong>, then click two points
                        along something with a known length (a labelled scale bar, a wall whose
                        length you know, etc.). This dialog will reopen so you can enter the real
                        distance.
                      </p>
                      <button
                    ...
                    <>
                      <p className="text-xs text-gray-500">
                        Drawn distance:{' '}
                        <strong>
                          {distPx(scaleDialog.points[0], scaleDialog.points[1]).toFixed(1)} px
                        </strong>
                      </p>
                      <div>
                        <label className="block text-xs font-semibold text-gray-600 mb-1">
                        ...
                        <div className="flex gap-2">
                          <input
                            ...
                            step="any"
                            autoFocus
                            value={scaleDialog.distance}
                            onChange={e =>
                              setScaleDialog({ ...scaleDialog, distance: e.target.value })
                            }
                ...
                <button
                  ...
                  disabled={
                    scaleDialog.mode === 'page'
                      ? !scaleDialog.preset
                      : !scaleDialog.points ||
                        !scaleDialog.distance ||
                        parseFloat(scaleDialog.distance) <= 0
                  }
                  className="px-5 py-2 rounded-lg text-sm font-bold text-white disabled:opacity-50"
                  style={{ backgroundColor: FG }}
                >
  ...
  if (a.type === 'area') {
    ...
    return (
      <g pointerEvents="none">
        ...
        {pts.map((p, i) => (
          <circle key={i} cx={p[0]} cy={p[1]} r={3} fill={color} stroke="#fff" strokeWidth={1} />
        ))}
        <rect
          x={cx - labelW / 2}
          y={cy - 10}
          ...
          stroke={color}
        />
        <text x={cx} y={cy + 5} textAnchor="middle" fontSize={11} fill="#1f2937" fontWeight={600}>
          {labelText}
        </text>
      </g>
    )
