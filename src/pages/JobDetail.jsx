...
export default function JobDetail() {
  ...
  useEffect(() => {
    if (activeTab === 'documents' && id) fetchFiles()
  }, [activeTab, id])
  ...
  async function fetchFiles() {
    ...
    if (data) {
      const withUrls = data.map(f => {
        const { data: urlData } = supabase.storage.from('job-files').getPublicUrl(f.storage_path)
  ...
  async function handleUpload(e) {
    ...
    const { error: storageErr } = await supabase.storage.from('job-files').upload(path, file)
    if (!storageErr) {
  ...
  const changeOrderRevenue = (job.change_orders || []).reduce(
  ...
  const coMD = (job.change_orders || []).reduce(
    (s, co) => s + parseFloat(co.additional_man_days || 0),
    0
  )
  const coMat = (job.change_orders || []).reduce(
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="card mb-4 bg-blue-50">
          ...
          <div className="flex flex-col items-end gap-2">
            {editingStatus ? (
              ...
              <button
                ...
                className={STATUS_LABELS[job.status] || 'badge-active'}
              >
                {job.status === 'on_hold'
                  ? 'On Hold'
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm text-gray-600 mb-4">
          {job.salesperson && (
            <div>
              <span className="font-medium text-gray-500 text-xs block">Salesperson</span>
              {job.salesperson}
            </div>
          )}
          {job.date_sold && (
            <div>
              <span className="font-medium text-gray-500 text-xs block">Date Sold</span>
              {new Date(job.date_sold).toLocaleDateString()}
            </div>
          )}
          {job.start_date && (
            <div>
              <span className="font-medium text-gray-500 text-xs block">Start Date</span>
              {new Date(job.start_date).toLocaleDateString()}
            </div>
          )}
          {job.estimated_completion && (
            <div>
              <span className="font-medium text-gray-500 text-xs block">Est. Completion</span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          ...
          <div
            className={`rounded-lg p-3 text-center ${totalEstGPPct >= 25 ? 'bg-green-50' : totalEstGPPct >= 10 ? 'bg-yellow-50' : 'bg-red-50'}`}
          >
            <p className="text-xs text-gray-500">Est. GP</p>
            <p
              className={`font-bold ${totalEstGPPct >= 25 ? 'text-green-700' : totalEstGPPct >= 10 ? 'text-yellow-700' : 'text-red-700'}`}
            >
            ...
            (job.projects || []).map(project => (
              <div key={project.id} className="card mb-3">
                ...
                {(project.modules || []).map(mod => {
                  const actualMD = (mod.actual_entries || []).reduce(
                    (s, e) => s + parseFloat(e.actual_man_days || 0),
                    0
                  )
                  const actualMat = (mod.actual_entries || []).reduce(
                  ...
                  return (
                    <div key={mod.id} className="bg-gray-50 rounded-lg p-3 mb-2">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full ${crewColors[mod.crew_type] || crewColors.General}`}
                        >
                          {mod.crew_type}
                        </span>
                      </div>
                      <div className="grid grid-cols-2 gap-x-4 text-xs text-gray-600">
                        <span>
                          Est. Man Days: <b>{parseFloat(mod.estimated_man_days || 0).toFixed(1)}</b>
                        </span>
                        <span>
                          Act. Man Days: <b>{actualMD.toFixed(1)}</b>
                        </span>
                        <span>
                          Est. Materials:{' '}
                          <b>${parseFloat(mod.estimated_material_cost || 0).toLocaleString()}</b>
                        </span>
                        <span>
                          Act. Materials: <b>${actualMat.toLocaleString()}</b>
                        </span>
                      </div>
                    </div>
                  )
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      type="number"
                      step="0.01"
                      value={coForm.additional_contract_price}
                      onChange={e =>
                        setCOForm(p => ({ ...p, additional_contract_price: e.target.value }))
                      }
                      placeholder="0.00"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      type="number"
                      step="0.25"
                      value={coForm.additional_man_days}
                      onChange={e =>
                        setCOForm(p => ({ ...p, additional_man_days: e.target.value }))
                      }
                      placeholder="0.0"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      type="number"
                      step="0.01"
                      value={coForm.additional_material_cost}
                      onChange={e =>
                        setCOForm(p => ({ ...p, additional_material_cost: e.target.value }))
                      }
                      placeholder="0.00"
                    />
                  </div>
                ...
                <div>
                  ...
                  <input
                    className="input"
                    type="date"
                    value={coForm.date_added}
                    onChange={e => setCOForm(p => ({ ...p, date_added: e.target.value }))}
                  />
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    value={coForm.notes}
                    onChange={e => setCOForm(p => ({ ...p, notes: e.target.value }))}
                ...
                <div className="flex gap-2">
                  ...
                  <button type="submit" disabled={savingCO} className="btn-primary flex-1">
            ...
            (job.change_orders || []).map(co => (
              <div key={co.id} className="card mb-2">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      {new Date(co.date_added).toLocaleDateString()}
                    </p>
                  </div>
                  <button
                ...
                <div className="grid grid-cols-3 gap-2 mt-2 text-xs text-gray-600">
                  <span>
                    +${parseFloat(co.additional_contract_price || 0).toLocaleString()} revenue
                  </span>
                  <span>+{parseFloat(co.additional_man_days || 0).toFixed(1)} man days</span>
                  <span>
                    +${parseFloat(co.additional_material_cost || 0).toLocaleString()} materials
                  </span>
                </div>
      ...
      {activeTab === 'documents' && (
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <label
              className={`cursor-pointer text-sm px-3 py-1.5 rounded-lg font-medium transition-colors ${uploading ? 'bg-gray-200 text-gray-400' : 'bg-green-700 text-white hover:bg-green-800'}`}
            >
          ...
          {filesLoading ? (
            ...
            <div className="card text-center py-10 text-gray-400">
              ...
              <p className="text-sm">
                No documents yet — upload a file or import from BuilderTrend
              </p>
            </div>
          ) : (
