...
export default function SubsVendors({ mode = 'sub' }) {
  ...
  function applyMappingAndPreview() {
    ...
    if (!m.company_name) {
      setImportError('You must map a column to Company Name.')
      return
    }
    setImportError('')
    const rows = importRawData
      .map((row, i) => {
        ...
        const status =
          STATUS_OPTIONS.find(
            s =>
              rawStatus.includes(s.value.replace('_', '')) ||
              rawStatus.includes(s.label.toLowerCase())
          )?.value || 'no_email'
        return {
          _row: i + 2,
          type: importType,
    ...
    if (!rows.length) {
      setImportError('No valid rows after mapping. Check that Company Name column has data.')
      return
    }
    setImportRows(rows)
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* ── Master Subcontractor Rates (subs only) ── */}
      {svTab === 'subRates' && mode === 'sub' && (
        <div className="mt-3 flex-1 overflow-y-auto">
          <MasterRates only="subs" />
        </div>
      )}
        ...
        <div className="-mb-6 mt-3 flex-1 flex flex-col">
          <div className="flex border border-gray-200 bg-white px-6 flex-nowrap overflow-x-auto flex-shrink-0 rounded-xl mb-3">
            {[
              ...
              <button
                key={t.key}
                onClick={() => setSvSettingsTab(t.key)}
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          {sub.primary_contact && (
                            <p className="text-xs text-gray-500 mt-0.5">{sub.primary_contact}</p>
                          )}
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full border font-medium flex-shrink-0 ${st.cls}`}
                        >
                          {st.label}
                        </span>
                      </div>
                      ...
                      {(liabExp || wcExp) && (
                        <p className="text-xs text-red-500 mt-2 font-medium">
                          ⚠️ {liabExp && 'Liability'}
                          {liabExp && wcExp && ' & '}
                          {wcExp && "Worker's Comp"} insurance expired
                        </p>
                      )}
                    </div>
                  )
              ...
              <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-[560px] flex flex-col max-h-[92vh]">
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 flex-shrink-0">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      {importMeta?.name} · {importMeta?.rowCount} rows · {importHeaders.length}{' '}
                      columns detected · importing as{' '}
                      <strong>{importType === 'sub' ? 'Subcontractors' : 'Vendors'}</strong>
                    </p>
                  </div>
                  ...
                  <div className="bg-blue-50 border border-blue-100 rounded-lg px-3 py-2.5 mb-3">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {importHeaders.map(h => (
                        <span
                          key={h}
                          className="text-[11px] bg-white border border-blue-200 text-blue-600 rounded px-1.5 py-0.5"
                        >
                          {h}
                        </span>
                      ))}
                    ...
                    <p className="text-[11px] text-blue-500 mt-2">
                      Use the dropdowns below to match each app field to your file's column.
                    </p>
                  </div>
                  {[
              ...
              <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-[760px] flex flex-col max-h-[92vh]">
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 flex-shrink-0">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      {importRows.length} records ready to import as{' '}
                      {importType === 'sub' ? 'Subcontractors' : 'Vendors'}
                    </p>
                  </div>
                  <button onClick={closeImport} className="text-gray-300 hover:text-gray-500 p-2">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      {importRows.map((row, i) => (
                        <tr key={i} className="hover:bg-gray-50">
                          ...
                          <td className="px-3 py-2">
                            <span
                              className={`px-1.5 py-0.5 rounded-full border text-[10px] font-medium ${statusInfo(row.status).cls}`}
                            >
